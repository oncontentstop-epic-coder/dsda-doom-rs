...
static dboolean P_GiveWeapon(player_t *player, weapontype_t weapon,
  ...
  if (heretic)
    return Heretic_P_GiveWeapon(player, weapon);
...
dboolean P_GivePower(player_t *player, int power) {
  static const int tics[NUMPOWERS] = {
      INVULNTICS, 1 /* strength */, INVISTICS,  IRONTICS,       1 /* allmap */,
      INFRATICS,  WPNLEV2TICS,      FLIGHTTICS, 1 /* shield */, 1 /* health2 */
  };

  if (heretic && tics[power] > 1 && power != pw_ironfeet &&
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher) {
  ...
  if (heretic)
    return Heretic_P_TouchSpecialThing(special, toucher);
  ...
  case SPR_MEDI:
    ...
    if (player->health < 50) // cph - 25 + the 25 just added, thanks to Quasar
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source,
  ...
  if (target->flags & MF_SKULLFLY) {
    if (target->type == HERETIC_MT_MINOTAUR)
      return;
  ...
  if (heretic && inflictor)
    ...
    case HERETIC_MT_MACEFX4: // Death ball
      if ((target->flags2 & MF2_BOSS) ||
          target->type == HERETIC_MT_HEAD) { // Don't allow cheap boss kills
        break;
    ...
    case HERETIC_MT_PHOENIXFX2: // Flame thrower
      if (target->player &&
          P_Random(pr_heretic) < 128) { // Freeze player for a bit
    ...
    case HERETIC_MT_PHOENIXFX1:
      if (target->type == HERETIC_MT_SORCERER2 &&
          P_Random(pr_heretic) < 96) { // D'Sparil teleports away
    ...
    case HERETIC_MT_RIPPER:
      if (target->type == HERETIC_MT_HEAD) { // Less damage to Ironlich bosses
  ...
  if (inflictor && !(target->flags & MF_NOCLIP) &&
    ...
    fixed_t thrust =
        damage * (FRACUNIT >> 3) * g_thrust_factor / target->info->mass;
  ...
  if (player) {
    ...
    if (!heretic && target->subsector->sector->special == 11 &&
    ...
    if (player->armortype) {
      ...
      if (heretic)
        saved = player->armortype == 1 ? (damage >> 1)
                                       : (damage >> 1) + (damage >> 2);
      else
  ...
  if (target->health <= 0) {
    if (heretic) {
      ...
      if (target->type == HERETIC_MT_POD && source &&
          source->type != HERETIC_MT_POD) { // Make sure players get frags for
                                            // chain-reaction kills
        P_SetTarget(&target->target, source);
      }
      if (player && inflictor &&
          !player->chickenTics) { // Check for flame death
  ...
  if (mbf_features) {
    ...
    if (target->health * 2 < target->info->spawnhealth) {
      thinker_t *cap =
          &thinkerclasscap[target->flags & MF_FRIEND ? th_friends : th_enemies];
  ...
  // e6y: Monsters could commit suicide in Doom v1.2 if they damaged themselves
  // by exploding a barrel
  if (source &&
      ...
      !(target->type == HERETIC_MT_SORCERER2 &&
        source->type == HERETIC_MT_WIZARD)) {
...
void A_RestoreSpecialThing1(mobj_t *thing) {
  if (thing->type == HERETIC_MT_WMACE) { // Do random mace placement

...
const switchlist_t heretic_alphSwitchList[] = {
    {"SW1OFF", "SW1ON", 1}, {"SW2OFF", "SW2ON", 1}, {"", "", 0}};
...
dboolean P_UseSpecialLine(mobj_t *thing, line_t *line, int side,
                          dboolean bossaction) {
  if (heretic)
    return Heretic_P_UseSpecialLine(thing, line, side, bossaction);
  ...
  if (bossaction) {
    switch (line->special) {
      // 0-tag specials, locked switches and teleporters need to be blocked for
      // boss actions.
    case 1:   // MANUAL DOOR RAISE
    case 32:  // MANUAL BLUE
    case 33:  // MANUAL RED

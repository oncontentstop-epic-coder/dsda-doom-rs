...
typedef struct {
  signed char
      istexture;   // jff 3/23/98 make char for comparison // cph - make signed
...
void P_InitPicAnims(void) {
  ...
  for (i = 0; animdefs[i].istexture != -1; i++) {
    ...
    if (animdefs[i].istexture) {
      ...
      if ((W_CheckNumForName)(animdefs[i].startname, ns_flats) ==
          -1) // killough 4/17/98
        continue;
    ...
    lastanim->speed =
        LittleLong(animdefs[i].speed); // killough 5/5/98: add LONG()
    lastanim++;
  }
  ...
  if (demo_compatibility &&
    ...
    for (i = 0, h = 0; i < sec->linecount; i++) {
      ...
      if (other->floorheight > height) {
        ...
        // 21: can be emulated;
        // 22..26: overflow affects saved registers - unpredictable behaviour,
        // can crash; 27: overflow affects return address - crash with high
        // probability;
        if (compatibility_level < dosdoom_compatibility &&
          ...
          lprintf(LO_WARN,
                  " Sector %d, line %d, heightlist index %d: ", sec->iSectorID,
          ...
          if (h <= MAX_ADJOINING_SECTORS + 1)
            ...
            lprintf(LO_WARN,
                    "cannot be emulated - crash with high probability.\n");
        }
    ...
    if (!h) {
      ...
      return (compatibility_level < doom_1666_compatibility ? last_height_0
                                                            : currentheight);
    }
  ...
  case AllKeys:
    ...
    if (skulliscard &&
        ((!player->cards[it_redcard] && !player->cards[it_redskull]) ||
         (!player->cards[it_bluecard] && !player->cards[it_blueskull]) ||
         // e6y
         // Compatibility with buggy MBF behavior when 3-key door works with
         // only 2 keys There is no more desync on 10sector.wad\ts27-137.lmp
         // http://www.doomworld.com/tas/ts27-137.zip
         (!player->cards[it_yellowcard] &&
          (compatibility_level == mbf_compatibility &&
                   !prboom_comp[PC_FORCE_CORRECT_CODE_FOR_3_KEYS_DOORS_IN_MBF]
                        .state
...
void P_CrossSpecialLine(line_t *line, int side, mobj_t *thing,
  ...
  if (heretic)
    return Heretic_P_CrossSpecialLine(line, side, thing);
  ...
  if (!demo_compatibility) // generalized types not recognized if old demo
  {
    ...
    if ((unsigned)line->special >= GenEnd) {
      ...
      if (!comperr(comperr_zerotag) &&
          !line->tag) // e6y //jff 2/27/98 all walk generalized types require
                      // tag
        return;
      ...
      if (((line->special & TriggerType) == WalkOnce) ||
          ((line->special & TriggerType) ==
           WalkMany)) { // jff 4/1/98 check for being a walk type before
                        // reporting door type
...
void P_PlayerInSpecialSector(player_t *player) {
  ...
  if (heretic)
    return Heretic_P_PlayerInSpecialSector(player);
  ...
  if (sector->special < 32) // regular sector specials
  {
    ...
    case 9:
      ...
      if (hudadd_secretarea) {
        int sfx_id =
            (I_GetSfxLumpNum(&S_sfx[sfx_secret]) < 0 ? sfx_itmbk : sfx_secret);
...
void P_SpawnSpecials(void) {
  ...
  for (i = 0; i < numsectors; i++, sector++) {
    ...
    switch (
        (demo_compatibility && !prboom_comp[PC_TRUNCATED_SECTOR_SPECIALS].state)
            ? sector->special
            : sector->special & 31) {
    case 1:
      // random off
  ...
  for (i = 0; i < numlines; i++)
    ...
    case 272: // Same, only flipped
              ...
              // e6y: sky property-transfer linedef types should be applied only for MBF
              and above         if (compatibility_level >= mbf_compatibility ||
...
void T_Scroll(scroll_t *s) {
  ...
  if (s->control !=
      -1) { // compute scroll amounts based on a sector's height changes
    fixed_t height =
  ...
  case sc_carry:
    ...
    waterheight =
        sec->heightsec != -1 && sectors[sec->heightsec].floorheight > height
...
static void P_SpawnScrollers(void) {
  ...
  for (i = 0; i < numlines; i++, l++) {
    ...
    case 255: // killough 3/2/98: scroll according to sidedef offsets
      s = lines[i].sidenum[0];
      Add_Scroller(sc_side, -sides[s].textureoffset, sides[s].rowoffset, -1, s,
                   accel);
      break;
...
static dboolean PIT_PushThing(mobj_t *thing) {
  ...
  if (!mbf_features
    ...
    if (speed > 0 && mbf_features) {
      ...
      speed =
          (int)(((uint_64_t)tmpusher->magnitude << 23) / (x * x + y * y + 1));
    }
...
int AmbSndSeq3[] = { // Drops
    afxcmd_play, heretic_sfx_amb3, afxcmd_delay, 16, afxcmd_delayrand, 31,
    afxcmd_play, heretic_sfx_amb7, afxcmd_delay, 16, afxcmd_delayrand, 31,
    afxcmd_play, heretic_sfx_amb3, afxcmd_delay, 16, afxcmd_delayrand, 31,
    afxcmd_play, heretic_sfx_amb7, afxcmd_delay, 16, afxcmd_delayrand, 31,
    afxcmd_play, heretic_sfx_amb3, afxcmd_delay, 16, afxcmd_delayrand, 31,
    afxcmd_play, heretic_sfx_amb7, afxcmd_delay, 16, afxcmd_delayrand, 31,

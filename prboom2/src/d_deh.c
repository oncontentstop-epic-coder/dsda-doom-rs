...
const char **const mapnames2[] = // DOOM 2 map names.
    {
        ...
        &s_HUSTR_12, &s_HUSTR_13, &s_HUSTR_14, &s_HUSTR_15, &s_HUSTR_16,
        &s_HUSTR_17, &s_HUSTR_18, &s_HUSTR_19, &s_HUSTR_20,

        &s_HUSTR_21, &s_HUSTR_22, &s_HUSTR_23, &s_HUSTR_24, &s_HUSTR_25,
        &s_HUSTR_26, &s_HUSTR_27, &s_HUSTR_28, &s_HUSTR_29, &s_HUSTR_30,
        &s_HUSTR_31, &s_HUSTR_32, &s_HUSTR_33,
};
...
const char **const mapnamesp[] = // Plutonia WAD map names.
    {
        ...
        &s_PHUSTR_12, &s_PHUSTR_13, &s_PHUSTR_14, &s_PHUSTR_15, &s_PHUSTR_16,
        &s_PHUSTR_17, &s_PHUSTR_18, &s_PHUSTR_19, &s_PHUSTR_20,
...
const char **const mapnamest[] = // TNT WAD map names.
    {
        ...
        &s_THUSTR_12, &s_THUSTR_13, &s_THUSTR_14, &s_THUSTR_15, &s_THUSTR_16,
        &s_THUSTR_17, &s_THUSTR_18, &s_THUSTR_19, &s_THUSTR_20,
...
static const deh_block deh_blocks[] = {
    ...
    /* 12 */ {"[CODEPTR]", deh_procBexCodePointers}, // bex codepointers by
                                                     // mnemonic
    /* 13 */ {"[HELPER]", deh_procHelperThing}, // helper thing substitution
                                                // haleyjd 9/22/99
...
static const struct deh_mobjflags_s deh_mobjflags[] = {
    ...
    {"TRANSLATION1",
     MF_TRANSLATION1}, // use translation table for color (players)
    {"TRANSLATION2",
     MF_TRANSLATION2}, // use translation table for color (players)
    {"UNUSED1",
     MF_TRANSLATION2},           // unused bit # 1 -- For Boom bug-compatibility
    ...
    {"BOUNCES",
     MF_BOUNCES},          // bounces off floors, ceilings and maybe walls (MBF)
...
static const deh_bexptr deh_bexptrs[] = // CPhipps - static const
    {
        ...
        {A_BetaSkullAttack, "A_BetaSkullAttack"}, // killough 10/98: beta lost
                                                  // souls attacked different
        {A_Stop, "A_Stop"},
  ...
  while (dehfgets(inbuffer, sizeof(inbuffer), filein)) {
    ...
    for (match = 0, i = 0; i < DEH_BLOCKMAX; i++)
      if (!strncasecmp(inbuffer, deh_blocks[i].key,
                       strlen(deh_blocks[i].key))) { // matches one
        if (i < DEH_BLOCKMAX - 1)
          match = 1;
...
static void deh_procBexCodePointers(DEHFILE *fpin, FILE *fpout, char *line) {
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    do      // Ty 05/16/98 - fix loop logic to look for null ending entry
    {
      ...
      if (!stricmp(
              key,
              deh_bexptrs[i].lookup)) { // Ty 06/01/98  - add  to
...
static uint_64_t getConvertedDEHBits(uint_64_t bits) {
  static const uint_64_t bitMap[32] = {
      ...
      MF_SPECIAL,   // 0 Can be picked up - When touched the thing can be picked
                    // up.
      MF_SOLID,     // 1 Obstacle - The thing is solid and will not let you (or
                    // others) pass through it
      ...
      MF_JUSTATTACKED, // 7 Steps before attack - Will take at least one step
                       // before attacking
      MF_SPAWNCEILING, // 8 Hangs from ceiling - When the level starts, this
                       // thing will be at ceiling height.
      MF_NOGRAVITY,    // 9 No gravity - Gravity does not affect this thing
      MF_DROPOFF, // 10 Travels over cliffs - Monsters normally do not walk off
                  // ledges/steps they could not walk up. With this set they can
                  // walk off any height of cliff. Usually only used for flying
                  // monsters.
      ...
      MF_TELEPORT, // 15 Semi no clipping - Don't cross lines or look at
                   // teleport heights. (don't really know much about this one
                   // either).
      MF_MISSILE,  // 16 Projectiles - Behaves like a projectile, explodes when
                   // hitting something that blocks movement
      MF_DROPPED, // 17 Disappearing weapon - Dropped, not spawned (like an ammo
                  // clip) I have not had much success in using this one.
      MF_SHADOW,  // 18 Partial invisibility - Drawn like a spectre.
      MF_NOBLOOD, // 19 Puffs (vs. bleeds) - If hit will spawn bullet puffs
                  // instead of blood splats.
      ...
      MF_COUNTKILL, // 22 Affects kill % - counted as a killable enemy and
                    // affects percentage kills on level summary.
      MF_COUNTITEM, // 23 Affects item % - affects percentage items gathered on
                    // level summary.
      ...
      MF_TRANSLUCENT // e6y: Translucency via dehacked/bex doesn't work without
                     // it
  };
  int i;
...
static void setMobjInfoValue(int mobjInfoIndex, int keyIndex, uint_64_t value) {
  mobjinfo_t *mi;
  if (mobjInfoIndex >= num_mobj_types || mobjInfoIndex < 0)
    return;
  ...
  case 22:
    if (prboom_comp[PC_FORCE_INCORRECT_PROCESSING_OF_RESPAWN_FRAME_ENTRY]
            .state) {
...
static void deh_procThing(DEHFILE *fpin, FILE *fpout, char *line) {
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    if (!bGetData)
    // Old code: if (!deh_GetData(inbuffer,key,&value,&strval,fpout)) // returns
    // TRUE if ok
    {
      if (fpout)
            ...
            for (iy = 0; iy < DEH_MOBJFLAGMAX; iy++) {
              ...
              if (fpout) {
                fprintf(fpout, "ORed value 0x%08lX%08lX %s\n",
                        (unsigned long)(deh_mobjflags[iy].value >> 32) &
                            0xffffffff,
...
static void deh_procFrame(DEHFILE *fpin, FILE *fpout, char *line) {
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    } else if (!deh_strcasecmp(
                   key,
                   deh_state[4])) // Codep frame (not set in Frame deh block)
    {
      if (fpout)
...
static void deh_procPointer(DEHFILE *fpin, FILE *fpout, char *line) // done
{
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    if (!deh_strcasecmp(
            key, deh_state[4])) // Codep frame (not set in Frame deh block)
    {
...
// Returns: void
//
static void deh_procSprite(DEHFILE *fpin, FILE *fpout,
                           char *line) // Not supported
{
  char key[DEH_MAXKEYLEN];
  char inbuffer[DEH_BUFFERMAX];
  int indexnum;
...
static void deh_procPars(DEHFILE *fpin, FILE *fpout, char *line) // extension
{
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
    ...
    if (!*inbuffer)
      break; // killough 11/98
    if (3 !=
        sscanf(inbuffer, "par %i %i %i", &episode, &level, &partime)) { // not 3
      ...
      // used, effectively making it a base 1 array. Ty 07/11/98 - level was
      // being checked against max 3 - dumb error Note that episode 4 does not
      // have par times per original design in Ultimate DOOM so that is not
      // supported here.
    ...
    for (ix = 0; cheat[ix].cheat; ix++)
      if (cheat[ix].deh_cheat) // killough 4/18/98: skip non-deh
      {
        if (!stricmp(key, cheat[ix].deh_cheat)) // found the cheat, ignored case
        {
          ...
          // with character 0xFF to the original cheat length, which we don't
          // do.
...
dboolean deh_GetData(char *s, char *k, uint_64_t *l, char **strval,
  ...
  // e6y: Correction of wrong processing of Bits parameter if its value is equal
  // to zero No more desync on HACX demos.

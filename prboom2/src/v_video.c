...
static const crdef_t crdefs[] = {
    {"CRBRICK", &colrngs[CR_BRICK]},   {"CRTAN", &colrngs[CR_TAN]},
...
static void FUNC_V_CopyRect(int srcscrn, int destscrn, int x, int y, int width,
  ...
  src =
      screens[srcscrn].data + screens[srcscrn].byte_pitch * y + x * pixel_depth;
  dest = screens[destscrn].data + screens[destscrn].byte_pitch * y +
         x * pixel_depth;
...
// In fact it packs pretty well, there is no big performance lose for all this
// merging; the inner loops themselves are just the same as they always were
// (indeed, laziness of the people who wrote the 'clones' of the original
// V_DrawPatch
//  means that their inner loops weren't so well optimised, so merging code may
//  even speed them).
//
static void V_DrawMemPatch(int x, int y, int scrn, const rpatch_t *patch,
  ...
  if (V_GetMode() == VID_MODE8 && !(flags & VPT_STRETCH_MASK)) {
    ...
    if (y < 0 ||
      ...
      lprintf(LO_WARN,
              "V_DrawMemPatch8: Patch (%d,%d)-(%d,%d) exceeds LFB in vertical "
              "direction (horizontal is clipped)\n"
    ...
    if (flags & VPT_TRANS) {
      colfunc = R_GetDrawColumnFunc(RDC_PIPELINE_TRANSLATED,
                                    drawvars.filterpatch, RDRAW_FILTER_NONE);
      dcvars.translation = trans;
    } else {
      colfunc = R_GetDrawColumnFunc(RDC_PIPELINE_STANDARD, drawvars.filterpatch,
                                    RDRAW_FILTER_NONE);
    }
    ...
    for (dcvars.x = left; dcvars.x <= right; dcvars.x++, col += DXI) {
      ...
      dcvars.texu =
          ((flags & VPT_FLIP) ? ((patch->width << FRACBITS) - col) : col) %
      ...
      for (i = 0; i < column->numPosts; i++) {
        ...
        if (!(flags & VPT_STRETCH_MASK)) {
          ...
          dcvars.yh = ((((y + post->topdelta + post->length) << 16) -
                        (FRACUNIT >> 1)) >>
                       FRACBITS);
        } else {
          // e6y
...
// CPhipps - some simple, useful wrappers for that function, for drawing patches
// from wads
...
void V_UpdateTrueColorPalette(video_mode_t mode) {
  ...
  gtable = (const byte *)W_CacheLumpNum(gtlump) +
  ...
  if (mode == VID_MODE32) {
    ...
    V_Palette32 =
        playpal_data->Palettes32 + paletteNum * 256 * VID_NUMCOLORWEIGHTS;
  } else if (mode == VID_MODE16) {
    if (!playpal_data->Palettes16) {
      // set short palette
      playpal_data->Palettes16 =
    ...
    V_Palette16 =
        playpal_data->Palettes16 + paletteNum * 256 * VID_NUMCOLORWEIGHTS;
  } else if (mode == VID_MODE15) {
    if (!playpal_data->Palettes15) {
      // set short palette
      playpal_data->Palettes15 =
          malloc(numPals * 256 * sizeof(short) * VID_NUMCOLORWEIGHTS);
      for (p = 0; p < numPals; p++) {
        for (i = 0; i < 256; i++) {
          r = gtable[pal[(256 * p + i) * 3 + 0]];
    ...
    V_Palette15 =
        playpal_data->Palettes15 + paletteNum * 256 * VID_NUMCOLORWEIGHTS;
  }
...
static void V_FillRect8(int scrn, int x, int y, int width, int height,
                        byte colour) {
...
static void V_FillRect15(int scrn, int x, int y, int width, int height,
                         byte colour) {
  unsigned short *dest =
      (unsigned short *)screens[scrn].data + x + y * screens[scrn].short_pitch;
  int w;
...
static void V_FillRect16(int scrn, int x, int y, int width, int height,
                         byte colour) {
  unsigned short *dest =
      (unsigned short *)screens[scrn].data + x + y * screens[scrn].short_pitch;
  int w;
...
static void V_FillRect32(int scrn, int x, int y, int width, int height,
                         byte colour) {
  unsigned int *dest =
      (unsigned int *)screens[scrn].data + x + y * screens[scrn].int_pitch;
  int w;
...
#ifdef GL_DOOM
static void WRAP_gld_FillRect(int scrn, int x, int y, int width, int height,
                              byte colour) {
...
static void NULL_FillRect(int scrn, int x, int y, int width, int height,
                          byte colour) {}
...
static void WRAP_V_DrawLine(fline_t *fl, int color) {
  ...
  // For debugging only
  if (fl->a.x < 0 || fl->a.x >= SCREENWIDTH || fl->a.y < 0 ||
      fl->a.y >= SCREENHEIGHT || fl->b.x < 0 || fl->b.x >= SCREENWIDTH ||
...
void V_FreePlaypal(void) { dsda_FreePlayPal(); }
...
void V_FillBorder(int lump, byte color) {
  ...
  if (lump >= 0) {
    if (bordtop > 0) {
      ...
      // Bottom
      V_FillFlat(lump, 0, 0, SCREENHEIGHT - bordbottom, SCREENWIDTH, bordbottom,
                 VPT_NONE);
    }
    if (bordleft > 0) {
      // Left
      V_FillFlat(lump, 0, 0, bordtop, bordleft,
    ...
    if (bordtop > 0) {
      ...
      // Bottom
      V_FillRect(0, 0, SCREENHEIGHT - bordbottom, SCREENWIDTH, bordbottom,
                 color);
    }
    if (bordleft > 0) {
      // Left
      V_FillRect(0, 0, bordtop, bordleft, SCREENHEIGHT - bordbottom - bordtop,
                 color);
      // Right
...
void SetRatio(int width, int height) {
  ...
  if ((width == 320 && height == 200) || (width == 640 && height == 400)) {
    lprintf(LO_INFO,
            "SetRatio: recognized VGA mode with pixel aspect ratio 5:6\n");
    ratio_multiplier = 4;
    ratio_scale = 3;
  } else {
  ...
  yaspectmul =
      Scale((320 << FRACBITS), WIDE_SCREENHEIGHT, 200 * WIDE_SCREENWIDTH);
  ...
  SCREEN_320x200 = (SCREENWIDTH == 320) && (SCREENHEIGHT == 200) &&
...
void V_DrawRawScreen(const byte *raw) { V_DrawRawScreenSection(raw, 0, 200); }

void V_DrawRawScreenSection(const byte *raw, int dest_y_offset,
                            int dest_y_limit) {
  int i, j;
...
//
//     col = 0;
//     desttop = dest_screen + ((y * dy) >> FRACBITS) * SCREENWIDTH + ((x * dx)
//     >> FRACBITS); desttop2 = dest_screen + (((y + 2) * dy) >> FRACBITS) *
//     SCREENWIDTH + (((x + 2) * dx) >> FRACBITS);
//
//     w = SHORT(patch->width);
...
//             SCREENWIDTH; dest2 = desttop2 + ((column->topdelta * dy) >>
//             FRACBITS) * SCREENWIDTH; count = (column->length * dy) >>
//             FRACBITS;
//
//             while (count--)
//             {
...
//             source = (byte *) column + 3;
//             dest = desttop + ((column->topdelta * dy) >> FRACBITS) *
//             SCREENWIDTH; count = (column->length * dy) >> FRACBITS;
//
//             while (count--)
//             {
//                 *dest = tinttable[((*dest) << 8) + source[srccol >>
//                 FRACBITS]]; srccol += dyi; dest += SCREENWIDTH;
//             }
//             column = (column_t *) ((byte *) column + column->length + 4);
//         }
//     }

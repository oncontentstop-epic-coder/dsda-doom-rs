...
menu_t ReadDef1 = {
    read1_end, &MainDef, ReadMenu1, M_DrawReadThis1, 330, 175,
...
void M_NewGame(int choice) {
  ...
  if (((gamemode == commercial || gamemission == chex) && !EpiCustom) ||
      EpiDef.numitems == 1)
    M_SetupNextMenu(&NewDef);
  else {
    epiChoice = 0;
...
menu_t LoadDef = {load_end,   &MainDef, LoadMenue,
                  M_DrawLoad, 80,       34, // jff 3/15/98 move menu up
                  0};
...
void M_SaveSelect(int choice) {
  ...
  if (!strcmp(savegamestrings[choice],
              s_EMPTYSTRING)) // Ty 03/27/98 - externalized
...
menu_t MouseDef = {
    mouse_end, &OptionsDef, MouseMenu, M_DrawMouse, 60, 37, // e6y
    0};
...
static void M_DrawItem(const setup_menu_t *s) {
  ...
  } else // Draw the item string
  {
    char *p, *t;
    int w = 0;
    int color = flags & S_DISABLE ? g_menu_cr_disable : // e6y
                    flags & S_SELECT ? g_menu_cr_select
                : flags & S_HILITE   ? g_menu_cr_hilite
                : flags & (S_TITLE | S_NEXT | S_PREV)
                    ? g_menu_cr_title
...
static void M_DrawSetting(const setup_menu_t *s) {
  ...
  color = flags & S_DISABLE ? g_menu_cr_disable : // e6y
              flags & S_SELECT ? g_menu_cr_select
          : flags & S_HILITE   ? g_menu_cr_hilite
                               : g_menu_cr_set;
  ...
  if (flags & S_COLOR) // Automap paint chip
  {
    ...
    // uses a patch draw the paint chip e6y: wide-res
    {
...
static void M_DrawInstructions(void) {
  ...
  if (setup_select) {
    switch (flags & (S_INPUT | S_YESNO | S_WEAP | S_NUM | S_COLOR | S_CRITEM |
                     S_CHAT | S_RESET | S_FILE | S_CHOICE)) {
    case S_INPUT:
...
setup_menu_t stat_settings2[] = {
    ...
    {"SHOW DEMOTIME",
     ...
     SB_Y + 6 * 8,
     {"hudadd_demotime"}},
    {"SHOW PROGRESS BAR DURING DEMO PLAYBACK",
     S_YESNO,
     m_null,
     ADVHUD_X,
     SB_Y + 7 * 8,
    ...
    {"CHANGE CROSSHAIR COLOR BY PLAYER HEALTH",
     S_YESNO,
     m_null,
     ADVHUD_X,
     SB_Y + 12 * 8,
     {"hudadd_crosshair_health"}},
    {"CHANGE CROSSHAIR COLOR ON TARGET",
     S_YESNO,
     m_null,
     ADVHUD_X,
     SB_Y + 13 * 8,
     {"hudadd_crosshair_target"}},
    {"LOCK CROSSHAIR ON TARGET",
     S_YESNO,
     m_null,
     ADVHUD_X,
     SB_Y + 14 * 8,
...
setup_menu_t auto_settings1[] = // 1st AutoMap Settings screen
    {{"Show Kills/Secrts/Items statistics",
      ...
      AU_Y + 0 * 8,
      {"map_level_stat"}},
     {"Show coordinates of automap pointer",
      S_YESNO,
      m_null,
      AU_X,
      AU_Y + 1 * 8,
      {"map_point_coord"}}, // killough 10/98
     {"Show Secrets only after entering",
      S_YESNO,
      m_null,
      AU_X,
      AU_Y + 2 * 8,
      {"map_secret_after"}},
     {"Update unexplored parts in automap mode",
      S_YESNO,
      m_null,
      AU_X,
      AU_Y + 3 * 8,
     ...
     {"Textured automap",
      ...
      AU_Y + 11 * 8,
      {"map_textured_trans"}},
     {"Textured automap in overlay mode",
      S_NUM,
      m_null,
      AU_X,
      AU_Y + 12 * 8,
      {"map_textured_overlay_trans"}},
     {"Lines in overlay mode",
      S_NUM,
      m_null,
      AU_X,
      AU_Y + 13 * 8,
...
setup_menu_t auto_settings2[] = // 2st AutoMap Settings screen
    {{"background", S_COLOR, m_null, AU_X, AU_Y, {"mapcolor_back"}},
     ...
     {"line at floor height change",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 3 * 8,
      {"mapcolor_fchg"}},
     {"line at ceiling height change",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 4 * 8,
      {"mapcolor_cchg"}},
     {"line at sector with floor = ceiling",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 5 * 8,
     ...
     {"AUTOMAP LEVEL TITLE COLOR",
      S_CRITEM,
      m_null,
      AU_X,
      AU_Y + 12 * 8,
      {"hudcolor_titl"}},
     {"AUTOMAP COORDINATES COLOR",
      S_CRITEM,
      m_null,
      AU_X,
      AU_Y + 13 * 8,
     ...
     {"Automap Statistics Titles Color",
      S_CRITEM,
      m_null,
      AU_X,
      AU_Y + 14 * 8,
      {"hudcolor_mapstat_title"}},
     {"Automap Statistics Values Color",
      S_CRITEM,
      m_null,
      AU_X,
      AU_Y + 15 * 8,
      {"hudcolor_mapstat_value"}},
     {"Automap Statistics Time Color",
      S_CRITEM,
      m_null,
      AU_X,
      AU_Y + 16 * 8,
...
setup_menu_t auto_settings3[] = // 3nd AutoMap Settings screen
    {{"teleporter line", S_COLOR, m_null, AU_X, AU_Y, {"mapcolor_tele"}},
     ...
     {"computer map unseen line",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 3 * 8,
      {"mapcolor_unsn"}},
     {"line w/no floor/ceiling changes",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 4 * 8,
     ...
     {"countable enemy sprite",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 6 * 8,
      {"mapcolor_enemy"}}, // cph 2006/06/30
     {"countable item sprite",
      S_COLOR,
      m_null,
      AU_X,
      AU_Y + 7 * 8,
...
setup_menu_t gen_settings1[] = { // General Settings screen1
    ...
    {"Software Exclusive Fullscreen",
     ...
     0,
     M_ChangeVideoMode},
    {"Status Bar and Menu Appearance",
     S_CHOICE,
     m_null,
     G_X,
     G_Y + 7 * 8,
    ...
    {"Enable Translucency",
     ...
     0,
     M_Trans},
    {"Translucency filter percentage",
     S_NUM,
     m_null,
     G_X,
     G_Y + 11 * 8,
...
setup_menu_t gen_settings5[] = { // General Settings screen3
    ...
    {"Multisampling (0-None)",
     S_NUM | S_PRGWARN | S_CANT_GL_ARB_MULTISAMPLEFACTOR,
     m_null,
     G_X,
     G_Y + 6 * 8,
...
setup_menu_t gen_settings6[] = {
    ...
    {"WARN ON MISSEDBACKSIDE OVERFLOW",
     S_YESNO,
     m_null,
     G_X2,
     G_Y + 12 * 8,
...
setup_menu_t gen_settings8[] = { // General Settings screen4
    ...
    {"Enable External Hi-Res",
     ...
     0,
     M_ChangeTextureUseHires},
    {"Override PWAD's graphics with Hi-Res",
     S_YESNO | S_PRGWARN,
     m_null,
     G_X,
     G_Y + 11 * 8,
...
setup_menu_t dsda_gen_settings[] = {
    ...
    {"Automatic Key Frame Interval (s)",
     S_NUM,
     m_null,
     G_X,
     G_Y + 4 * 8,
     {"dsda_auto_key_frame_interval"}},
    {"Automatic Key Frame Depth",
     S_NUM | S_PRGWARN,
     m_null,
     G_X,
     G_Y + 5 * 8,
...
void M_Trans(void) // To reset translucency after setting it in menu
{
  general_translucency =
      default_translucency; // e6y: Fix for "translucency won't change until you
                            // restart the engine"
...
setup_menu_t mess_settings1[] = // Messages screen
  ...
  {"Chat Message Duration (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_chat_timer*8, {"chat_msg_timer"}},
     ...
     {"Message Review Color",
      S_CRITEM,
      m_null,
      M_X,
      M_Y + mess_color_review * 8,
      {"hudcolor_list"}},
  ...
  {"Message Review Duration (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_hud_timer*8, {"hud_msg_timer"}},
     ...
     {"Number of Review Message Lines",
      S_NUM,
      m_null,
      M_X,
      M_Y + mess_lines * 8,
...
static setup_menu_t **setup_screens[] = {
    keys_settings, weap_settings, stat_settings,
    auto_settings, mess_settings, chat_settings,
...
static int M_GetPixelWidth(const char *ch) {
  ...
  len -=
      g_menu_font_spacing; // replace what you took away on the last char only
  return len;
}
...
setup_menu_t cred_settings[] = {
    ...
    {"The DOSDoom-Team for DOSDOOM", S_SKIP | S_CREDIT | S_LEFTJUST, m_null,
     CR_X2, CR_Y + CR_S *(adcr + 4) + CR_SH *cr_adcr},
    {"Randy Heit for ZDOOM", S_SKIP | S_CREDIT | S_LEFTJUST, m_null, CR_X2,
     CR_Y + CR_S *(adcr + 5) + CR_SH *cr_adcr},
    {"Michael 'Kodak' Ryssen for DOOMGL", S_SKIP | S_CREDIT | S_LEFTJUST,
     m_null, CR_X2, CR_Y + CR_S *(adcr + 6) + CR_SH *cr_adcr},
    {"Jess Haas for lSDLDoom", S_SKIP | S_CREDIT | S_LEFTJUST, m_null, CR_X2,
     CR_Y + CR_S *(adcr + 7) + CR_SH *cr_adcr},
    {"all others who helped (see AUTHORS file)", S_SKIP | S_CREDIT | S_LEFTJUST,
     m_null, CR_X2, CR_Y + CR_S *(adcr + 8) + CR_SH *cr_adcr},
...
void M_DrawCredits(void) // killough 10/98: credit screen
{
  ...
  if (creditlump >= 0 && lumpinfo[creditlump].source != source_iwad) {
    ...
    M_DrawTitle(81, 9, "PRBOOM", g_cr_gold, PACKAGE_NAME " v" PACKAGE_VERSION,
                g_cr_gold);
...
dboolean M_Responder(event_t *ev) {
  ...
  ch = MENU_NULL; // will be changed to a legit char if we're going to use it
                  // here
  action = MENU_NULL; // differentiate between action caused by a binding and
                      // input ch
...
// e6y
    ...
    if (dsda_InputActivated(dsda_input_zoomin)) { // jff 2/23/98
    ...
    if (dsda_InputActivated(dsda_input_speed_default) &&
        (!netgame || demoplayback) && !dsda_StrictMode()) {
      realtic_clock_rate =
          StepwiseSum(realtic_clock_rate, 0, speed_step, 3, 10000, 100);
      I_Init2();
    ...
    if (dsda_InputActivated(dsda_input_speed_up) &&
        (!netgame || demoplayback) && !dsda_StrictMode()) {
      realtic_clock_rate =
          StepwiseSum(realtic_clock_rate, 1, speed_step, 3, 10000, 100);
      I_Init2();
      // Don't eat the keypress in this case.
      // return true;
    }
      ...
      if (ptr1->m_flags & S_YESNO) // yes or no setting?
      {
        if (action == MENU_ENTER) {
          *ptr1->var.def->location.pi =
              !*ptr1->var.def->location.pi; // killough 8/15/98
...
// e6y
    ...
    if (setup_select && set_general_active | set_chat_active | set_mess_active |
                            set_status_active) {
...
void M_Drawer(void) {
  ...
  if (messageToPrint) {
    ...
    if (heretic)
      return MN_DrawMessage(messageString);
    ...
    if (lumps_missing == 0)
      for (i = 0; i < max; i++) {
        if (currentMenu->menuitems[i].name[0])
          V_DrawNamePatch(x, y, 0, currentMenu->menuitems[i].name, CR_DEFAULT,
                          VPT_STRETCH);
        y += LINEHEIGHT;
      }
    else
...
void M_DrawThermo(int x, int y, int thermWidth, int thermDot) {
  ...
  if (heretic)
    return MN_DrawSlider(x, y, thermWidth, thermDot);
...
void M_DrawTitle(int x, int y, const char *patch, int cm, const char *alttext,
  ...
  if (lumpnum >= 0) {
    ...
    M_WriteText(160 - (M_StringWidth(alttext) / 2),
                y + 8 -
                    (M_StringHeight(alttext) / 2), // assumes patch height 16
                alttext, altcm);
  }
}

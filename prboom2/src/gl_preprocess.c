  ...
  for (i = 0; i < numclippers; i++) {
    ...
    for (k = 0; k < num; k++) {
      ...
      if (sidelist[startIdx] != sidelist[endIdx]) {
        ...
        gld_CalcIntersectionVertex(&points[startIdx], &points[endIdx], curclip,
                                   &newvert);
...
static void gld_FlatConvexCarver(int ssidx, int num, divline_t *list) {
  ...
  clippers =
      (divline_t *)Z_Malloc(numclippers * sizeof(divline_t), PU_LEVEL, 0);
  if (!clippers)
    return;
  for (i = 0; i < num; i++) {
  ...
  numedgepoints = 4;
  edgepoints =
      (vertex_t *)Z_Malloc(numedgepoints * sizeof(vertex_t), PU_LEVEL, 0);
  ...
  edgepoints =
      gld_FlatEdgeClipper(&numedgepoints, edgepoints, numclippers, clippers);
...
static void gld_CarveFlats(int bspnode, int numdivlines, divline_t *divlines) {
  ...
  childlist =
      (divline_t *)Z_Malloc(childlistsize * sizeof(divline_t), PU_LEVEL, 0);
...
static void CALLBACK ntessBegin(GLenum type) {
  ...
  sectorloops[currentsector].loops = Z_Realloc(
...
// glu tesselation functions. the rest of this stuff is entirely done by me
// (proff). if there are any similarities, then they are implications of the
// algorithm.
//
// I'm starting with the first line of the current sector. I take it's ending
// vertex and add it to the tesselator. the current line is marked as used. then
// I'm searching for the next line which connects to the current line. if there
// is more than one line, I choose the one with the smallest angle to the
// current. if there is no next line, I start a new loop and take the first
// unused line in the sector. after all lines are processed, the polygon is
// tesselated.
//
// e6y
// The bug in algorithm of splitting of a sector into the closed contours was
// fixed. There is no more HOM at the starting area on MAP16 @ Eternal.wad I
...
static void gld_PrecalculateSector(int num) {
    ...
    if (sectors[num].lines[i]->sidenum[0] != NO_INDEX)
      if (sectors[num].lines[i]->sidenum[1] != NO_INDEX)
        if (sides[sectors[num].lines[i]->sidenum[0]].sector ==
          ...
          if (levelinfo)
            fprintf(levelinfo,
                    "line %4i (iLineID %4i) has both sides in same sector "
                    "(removed)\n",
  ...
  for (i = 0; i < sectors[num].linecount - 1; i++) {
    ...
    for (j = i + 1; j < sectors[num].linecount; j++) {
      if (sectors[num].lines[i]->v1 == sectors[num].lines[j]->v1 &&
          ...
          sectors[num].lines[i]->frontsector ==
              sectors[num].lines[j]->frontsector &&
          sectors[num].lines[i]->backsector ==
              sectors[num].lines[j]->backsector &&
  ...
  while (linecount) {
      ...
      for (i = 0; i < sectors[num].linecount; i++)
        if (!lineadded[i]) {
          ...
          if ((sectors[num].lines[currentline]->sidenum[0] != NO_INDEX)
                  ? (sides[sectors[num].lines[currentline]->sidenum[0]]
                         .sector == &sectors[num])
                  : false)
    ...
    if ((sectors[num].lines[currentline]->sidenum[0] != NO_INDEX)
      ...
      if (levelinfo)
        fprintf(levelinfo,
                "\t\tAdded Line %4i to Loop, iLineID %5i, Angle: %4i, flipped "
                "false\n",
    ...
    // add the vertex to the tesselator, currentvertex is the pointer to the
    ...
    linecount--;
    // find the next line
    oldline = currentline; // if this isn't changed at the end of the search, a
                           // new loop will start
        ...
        if ((sectors[num].lines[i]->v1 == currentvertex) ||
          ...
          if ((sectors[num].lines[i]->sidenum[0] != NO_INDEX)
            ...
            angle = R_PointToAngle2(
                sectors[num].lines[i]->v1->x, sectors[num].lines[i]->v1->y,
                sectors[num].lines[i]->v2->x, sectors[num].lines[i]->v2->y);
          else
            angle = R_PointToAngle2(
                sectors[num].lines[i]->v2->x, sectors[num].lines[i]->v2->y,
                sectors[num].lines[i]->v1->x, sectors[num].lines[i]->v1->y);
          ...
          if ((sectors[num].lines[i]->sidenum[0] != NO_INDEX)
                  ? (sides[sectors[num].lines[i]->sidenum[0]].sector ==
                     &sectors[num])
                  : false) {
            // when the line is not flipped and startvertex is not the
            // currentvertex then skip this line
            if (sectors[num].lines[i]->v1 != currentvertex)
              continue;
          } else {
            // when the line is flipped and endvertex is not the currentvertex
            // then skip this line
          ...
          if (bestline == -1) // if this is the first one ...
          {
            ...
            bestlinecount++;
              ...
              // check if the angle between the current line and this best line
              // candidate is smaller then the angle of the last candidate e6y:
              // for finding an angle between AB and BC vectors we should
              // subtract (BC - BA) == (BC - (180 - AB)) ==
              // (angle-(180-lineangle))
...
// gld_PreprocessLevel
//
// this checks all sectors if they are closed and calls gld_PrecalculateSector
// to calculate the loops for every sector the idea to check for closed sectors
// is from DEU. check next commentary
/*
      Note from RQ:
...
static void gld_PreprocessSectors(void) {
  ...
  if (numsubsectors) {
    ...
    if (!subsectorloops)
      I_Error(
          "gld_PreprocessSectors: Not enough memory for array subsectorloops");
  ...
  if (numlines) {
    ...
    if (!linerendered[0] || !linerendered[1])
      I_Error(
          "gld_PreprocessSectors: Not enough memory for array linerendered");
  }
  ...
  for (i = 0; i < numsectors; i++) {
    ...
    for (j = 0; j < sectors[i].linecount; j++) {
      v1num = ((intptr_t)sectors[i].lines[j]->v1 - (intptr_t)vertexes) /
              sizeof(vertex_t);
  ...
  if (!gl_preprocessed) {
    if (gl_ext_arb_vertex_buffer_object) {
      ...
      GLEXT_glBufferDataARB(GL_ARRAY_BUFFER,
                            gld_num_vertexes * sizeof(flats_vbo[0]), flats_vbo,
                            GL_STATIC_DRAW_ARB);

...
void D_PostEvent(event_t *ev) {
  ...
  if (doSkip) {
    if (dsda_InputActivated(dsda_input_quit)) {
      ...
      if (!dsda_InputActivated(dsda_input_use) &&
          !dsda_InputActivated(dsda_input_demo_skip) &&
          (ev->type == ev_keydown ||
           ev->type == ev_keyup) // is this condition important?
      ) {
        return;
      }
    }
...
void D_Display(fixed_t frac) {
  ...
  if (gamestate != GS_LEVEL) { // Not a level
    ...
    if (oldgamestate != GS_LEVEL) {
      ...
      // screen, and there is a menu being displayed
  ...
  if (paused && (menuactive != mnact_full)) {
    if (heretic)
      ...
      V_DrawNamePatch((320 - V_NamePatchWidth("M_PAUSE")) / 2, 4, 0, "M_PAUSE",
...
static void D_DrawTitle2(const char *name) { D_SetPage(name, 0, mus_dm2ttl); }
...
static void IdentifyVersion(void) {
  ...
  if ((i = M_CheckParm("-save")) &&
      i < myargc - 1) // jff 3/24/98 if -save present
  {
...
static void FindResponseFile(void) {
  ...
  for (i = 1; i < myargc; i++)
    if (myargv[i][0] == '@') {
      ...
      if (size < 0) {
        size_t fnlen =
            doom_snprintf(NULL, 0, "%s/%s", I_DoomExeDir(), &myargv[i][1]);
...
static void DoLooseFiles(void) {
  ...
  struct {
    ...
    int *count;
                ...
                {".deh", &dehs, &dehcount},
                {".bex", &dehs, &dehcount},
                // assume wad if no extension or length of the extention is not
                // equal to 3 must be last entrie
                {"", &wads, &wadcount},
                {0}};
    ...
    while (params[k].cmdparam) {
      ...
      if (*(params[k].count) > 0) {
        ...
        for (i = 0; i < *(params[k].count);)
          tmyargv[tmyargc++] =
              (*(params[k].list))[i++]; // allocated by strdup above
      }
      k++;
    }
...
static void L_SetupConsoleMasks(void) {
  int p;
  int i;
  const char *cena = "ICWEFDA", *pos; // jff 9/3/98 use this for parsing console
                                      // masks // CPhipps - const char*'s
...
static void D_DoomMainSetup(void) {
...

  {
    ...
    if (bfgedition) {
      ...
      tempverstr =
          malloc(sizeof(char) * (strlen(doomverstr) + strlen(bfgverstr) + 1));
    ...
    lprintf(LO_ALWAYS,
            ...
            "It comes with ABSOLUTELY NO WARRANTY. See the file COPYING for "
            "details.\n",
  ...
  if ((p = M_CheckParm("-warp")) || // killough 5/2/98
      (p = M_CheckParm("-wart")))
  // Ty 08/29/98 - moved this check later so we can have -warp alone: && p <
  // myargc-1)
  {
  ...
  // e6y: some stuff from command-line should be initialised before
  // ProcessDehFile()
  e6y_InitCommandLine();
  ...
  if (slot && ++slot < myargc) {
    ...
    G_LoadGame(slot,
               true); // killough 5/15/98: add command flag // cph - no filename

...
float
    gl_sprite_offset; // precalcilated float value for gl_sprite_offset_default
...
void gld_InitTextureParams(void) {
  ...
  tex_filter_t params[filter_count] = {
      ...
      {true, GL_NEAREST, GL_NEAREST_MIPMAP_LINEAR, "GL_NEAREST",
       "GL_NEAREST_MIPMAP_LINEAR"},
      {true, GL_LINEAR, GL_LINEAR_MIPMAP_NEAREST, "GL_LINEAR",
       "GL_LINEAR_MIPMAP_NEAREST"},
      {true, GL_LINEAR, GL_LINEAR_MIPMAP_LINEAR, "GL_LINEAR",
...
static int C_DECL dicmp_visible_subsectors_by_pic(const void *a,
                                                  const void *b) {
...
void gld_MapDrawSubsectors(player_t *plr, int fx, int fy, fixed_t mx,
  ...
  glTranslatef((float)fx -
                   (float)mx / (float)FRACUNIT * (float)scale / (float)FRACUNIT,
               (float)fy + (float)fh +
                   (float)my / (float)FRACUNIT * (float)scale / (float)FRACUNIT,
               0);
  coord_scale =
      (float)scale / (float)(1 << FRACTOMAPBITS) / (float)FRACUNIT * MAP_COEFF;
...
void gld_DrawNumPatch_f(float x, float y, int lump, int cm,
  ...
  if (flags & VPT_STRETCH_MASK) {
    ...
    xpos = (float)((x - leftoffset) * params->video->width) / 320.0f +
           params->deltax1;
    ypos = (float)((y - topoffset) * params->video->height) / 200.0f +
           params->deltay1;
  ...
  bFakeColormap = (gltexture->flags & GLTEXTURE_HIRES) &&
...
void gld_DrawLine_f(float x0, float y0, float x1, float y1, int BaseColor) {
  ...
  float alpha =
      ((automapmode & am_overlay) ? map_lines_overlay_trans / 100.0f : 1.0f);
  if (alpha == 0)
    return;
...
void gld_SetPalette(int palette) {
  ...
  if (gl_shared_texture_palette) {
    ...
    GLEXT_glColorTableEXT(GL_SHARED_TEXTURE_PALETTE_EXT, GL_RGBA, 256, GL_RGBA,
...
unsigned char *gld_ReadScreen(void) { // NSM convert to static
  ...
  if (buffer && scr) {
    ...
    for (i = 0; i < SCREENHEIGHT / 2; i++) {
      ...
      memcpy(&scr[(SCREENHEIGHT - (i + 1)) * SCREENWIDTH * 3], buffer,
             SCREENWIDTH * 3);
    }
  }
...
void gld_StartDrawScene(void) {
  ...
  glScissor(viewwindowx, SCREENHEIGHT - (viewheight + viewwindowy), viewwidth,
            viewheight);
  glEnable(GL_SCISSOR_TEST);
  // Player coordinates
...
void gld_EndDrawScene(void) {
  ...
  if (SceneInTexture) {
    ...
    if (motion_blur.enabled) {
      ...
      if (realframe) {
        motionblur_alpha = (float)((atan(-renderer_fps / motion_blur.att_a)) /
                                   motion_blur.att_b) +
                           motion_blur.att_c;
      }
...
static void gld_AddDrawWallItem(GLDrawItemType itemtype, void *itemdata) {
  if (gl_blend_animations) {
    ...
    case GLDIT_FWALL:
      ...
      if (anim) {
        wall->alpha = 1.0f - ((float)tic_vars.frac +
...
static void gld_DrawWall(GLWall *wall) {
  ...
  has_detail =
      scene_has_details && gl_arb_multitexture && (wall->flag < GLDWF_SKY) &&
...
#define CALC_Y_VALUES(w, lineheight, floor_height, ceiling_height)             \
  ...
  lineheight = ((float)fabs(((ceiling_height) / (float)FRACUNIT) -             \
                            ((floor_height) / (float)FRACUNIT)))

#define OU(w, seg)                                                             \
  (((float)((seg)->sidedef->textureoffset) / (float)FRACUNIT) /                \
   (float)(w).gltexture->buffer_width)
#define OV(w, seg)                                                             \
  (((float)((seg)->sidedef->rowoffset) / (float)FRACUNIT) /                    \
   (float)(w).gltexture->buffer_height)
#define OV_PEG(w, seg, v_offset)                                               \
  (OV((w), (seg)) - (((float)(v_offset) / (float)FRACUNIT) /                   \
                     (float)(w).gltexture->buffer_height))
...
#define CALC_TEX_VALUES_TOP(w, seg, backseg, peg, linelength, lineheight)      \
  ...
  if (peg) {                                                                   \
    ...
    (w).vt = ((w).vb -                                                         \
              ((float)(lineheight) / (float)(w).gltexture->buffer_height));    \
  } else {                                                                     \
    (w).vt = OV((w), (seg));                                                   \
    (w).vb =                                                                   \
        (w).vt + ((float)(lineheight) / (float)(w).gltexture->buffer_height);  \
  }
...
#define CALC_TEX_VALUES_BOTTOM(w, seg, backseg, peg, linelength, lineheight,   \
                               v_offset)                                       \
  (w).flag = GLDWF_BOT;                                                        \
  URUL(w, seg, backseg, linelength);                                           \
  if (peg) {                                                                   \
...
void gld_AddWall(seg_t *seg) {
  ...
  if (fake_contrast) {
    rellight = seg->linedef->dx == 0   ? +gl_rellight
               : seg->linedef->dy == 0 ? -gl_rellight
                                       : 0;
  }
  wall.light = gld_CalcLightLevel(frontsector->lightlevel + rellight +
                                  (extralight << 5));
  ...
  if (!seg->backsector) /* onesided */
  {
    ...
    temptex = gld_RegisterTexture(texturetranslation[seg->sidedef->midtexture],
                                  true, false);
    if (temptex && frontsector->ceilingheight > frontsector->floorheight) {
      wall.gltexture = temptex;
      CALC_Y_VALUES(wall, lineheight, frontsector->floorheight,
                    frontsector->ceilingheight);
      CALC_TEX_VALUES_MIDDLE1S(wall, seg, backseg,
                               (LINE->flags & ML_DONTPEGBOTTOM) > 0, linelength,
                               lineheight);
  ...
  } else /* twosided */
  {
    ...
    if (frontsector->ceilingpic ==
        skyflatnum) // || backsector->ceilingpic==skyflatnum)
    {
      wall.ytop = MAXCOORD;
      if (
          // e6y
          // There is no more HOM in the starting area on Memento Mori map29 and
          // on map30. Old code:
        ...
        wall.ybottom =
            ((float)(backsector->floorheight + (seg->sidedef->rowoffset > 0
    ...
    if (floor_height < ceiling_height) {
      if (!((frontsector->ceilingpic == skyflatnum) &&
        ...
        if (!temptex && gl_use_stencil && backsector &&
          ...
          wall.ybottom =
              ((float)(floor_height) / (float)MAP_SCALE) - SMALLDELTA;
    ...
    if (temptex && seg->sidedef->midtexture != NO_TEXTURE &&
      ...
      if ((LINE->flags & ML_DONTPEGBOTTOM) > 0) {
        ...
        floor_height =
            MAX(seg->frontsector->floorheight, seg->backsector->floorheight) +
            (seg->sidedef->rowoffset);
        ceiling_height =
            floor_height + (wall.gltexture->realtexheight << FRACBITS);
      } else {
        // ceiling_height=min_ceiling_tex;
        ceiling_height = MIN(seg->frontsector->ceilingheight,
    ...
    if (floor_height < ceiling_height) {
      ...
      if (!temptex && gl_use_stencil && backsector &&
        ...
        if (wall.ytop <= zCamera) {
          ...
          temptex = gld_RegisterFlat(flattranslation[seg->backsector->floorpic],
                                     true);
          if (temptex) {
            wall.gltexture = temptex;
            gld_AddDrawWallItem(GLDIT_FWALL, &wall);
...
static void gld_DrawFlat(GLFlat *flat) {
  ...
  has_detail =
      scene_has_details && gl_arb_multitexture && flat->gltexture->detail;
  ...
  if (flat->sectornum >= 0) {
    ...
    if (gl_use_display_lists) {
      int display_list =
          (has_detail ? flats_detail_display_list : flats_display_list);
...
// The ceiling boolean indicates if the flat is a floor(false) or a
// ceiling(true)
...
static void gld_AddFlat(int sectornum, dboolean ceiling, visplane_t *plane) {
  ...
  if (!ceiling) // if it is a floor ...
  {
    ...
    flat.fogdensity =
        gld_CalcFogDensity(sector, plane->lightlevel, GLDIT_FLOOR);
    ...
    if (sector->floor_xoffs | sector->floor_yoffs) {
      ...
      if (heretic) {
        ...
        case 24: // Scroll_East
          flat.flags |= GLFLAT_HAVE_OFFSET;
          flat.uoffs =
              (float)((63 - ((leveltime >> 1) & 63)) << (plane->special - 20) &
                      63) /
              64;
          break;
  ...
  } else // if it is a ceiling ...
  {
    ...
    flat.light = gld_CalcLightLevel(plane->lightlevel + (extralight << 5));
    flat.fogdensity =
        gld_CalcFogDensity(sector, plane->lightlevel, GLDIT_CEILING);
    // calculate texture offsets
  ...
  if (gl_blend_animations) {
    ...
    if (anim) {
      ...
      flat.alpha = 1.0f - ((float)tic_vars.frac +
                           ((leveltime - 1) % anim->speed) * 65536.0f) /
                              (65536.0f * anim->speed);
      gld_AddDrawItem(
          ((flat.flags & GLFLAT_CEILING) ? GLDIT_ACEILING : GLDIT_AFLOOR),
          &flat);
...
void gld_ProjectSprite(mobj_t *thing, int lightlevel) {
  ...
  if ((gl_spriteclip != spriteclip_const) && (sprite.y2 < 0) &&
      ...
      !(thing->flags &
        (MF_SPAWNCEILING | MF_FLOAT | MF_MISSILE | MF_NOGRAVITY)) &&
  ...
  if (frustum_culling) {
    if (!gld_SphereInFrustum(
            ...
            (float)(MAX(patch->width, patch->height)) / MAP_COEFF / 2.0f *
                1.5f)) {
      goto unlock_patch;
    }
  }
  ...
  if ((thing->frame & FF_FULLBRIGHT) || show_alive) {
    ...
    sprite.fogdensity =
        gld_CalcFogDensity(thing->subsector->sector, lightlevel, GLDIT_SPRITE);
  }
  sprite.cm =
      CR_LIMIT + (int)((thing->flags & MF_TRANSLATION) >> (MF_TRANSSHIFT));
  ...
  if (sprite.flags & MF_NO_DEPTH_TEST) {
    ...
    gld_AddDrawItem(
        (gl_sprite_blend || (sprite.flags & (MF_SHADOW | MF_TRANSLUCENT))
             ? GLDIT_TSPRITE
             : GLDIT_SPRITE),
        &sprite);
...
void gld_ProcessWall(GLWall *wall) {
  // e6y
  // The ultimate 'ATI sucks' fix: Some of ATIs graphics cards are so unprecise
  // when rendering geometry that each and every border between polygons must be
  // seamless, otherwise there are rendering artifacts worse than anything that
  // could be seen on Geforce 2's! Made this a menu option because the speed
  // impact is quite severe and this special handling is not necessary on modern
  // NVidia cards.
  seg_t *seg = wall->seg;
...
static void gld_DrawItemsSort(GLDrawItemType itemtype,
                              int(C_DECL *PtFuncCompare)(const void *,
                                                         const void *)) {
  qsort(gld_drawinfo.items[itemtype], gld_drawinfo.num_items[itemtype],
    ...
    if (!no_overlapped_sprites) {
      ...
      while (i < count) {
        ...
        if (sprite1->xy == sprite2->xy) {
          GLSprite *sprite =
              (sprite1->index > sprite2->index ? sprite1 : sprite2);
          i++;
          while (i < count && gld_drawinfo.items[itemtype][i].item.sprite->xy ==
                                  sprite1->xy) {
            if (gld_drawinfo.items[itemtype][i].item.sprite->index >
                sprite->index) {
    ...
    for (i = gld_drawinfo.num_items[itemtype] - 1; i >= 0; i--) {
      ...
      if (gl_use_fog) {
        ...
        if (wall->seg->backsector) {
          wall->fogdensity =
              gld_CalcFogDensity(wall->seg->frontsector,
...
void gld_DrawScene(player_t *player) {
  ...
  if (!skybox &&
      (gl_drawskys == skytype_none || gl_drawskys == skytype_standard)) {
  ...
  if (gld_drawinfo.num_items[GLDIT_TWALL] > 0 ||
    ...
    if (gld_drawinfo.num_items[GLDIT_TSPRITE] > 0) {
      ...
      for (i = gld_drawinfo.num_items[GLDIT_TSPRITE] - 1; i >= 0; i--) {
        gld_SetFog(
            gld_drawinfo.items[GLDIT_TSPRITE][i].item.sprite->fogdensity);

...
char savedescription[SAVEDESCLEN]; // Description to save in savegame if
...
void G_BuildTiccmd(ticcmd_t *cmd) {
  ...
  if (joyxmove != 0 || dsda_InputActive(dsda_input_turnright) ||
  ...
  if (heretic) {
    ...
    if (dsda_InputActive(dsda_input_lookdown) ||
        dsda_InputActive(dsda_input_lookup)) {
      lookheld += ticdup;
    } else {
      lookheld = 0;
    }
  ...
  if (dsda_InputActive(dsda_input_use) ||
      dsda_InputTickActivated(dsda_input_use)) {
    cmd->buttons |= BT_USE;
  ...
  if ((!demo_compatibility && players[consoleplayer].attackdown && // killough
    ...
    if (next_weapon) {
      ...
      newweapon =
          dsda_InputActive(dsda_input_weapon1) ? wp_fist
          ...
          : dsda_InputActive(dsda_input_weapon6) && gamemode != shareware
              ? wp_plasma
          : dsda_InputActive(dsda_input_weapon7) && gamemode != shareware
              ? wp_bfg
          : dsda_InputActive(dsda_input_weapon8) ? wp_chainsaw
          : (!demo_compatibility && dsda_InputActive(dsda_input_weapon9) &&
             gamemode == commercial)
              ? wp_supershotgun
              : wp_nochange;
    }
  ...
  if (mouse_doubleclick_as_use) { // e6y
    ...
    if (dsda_InputMouseBActive(dsda_input_forward) != dclickstate &&
        dclicktime > 1) {
    ...
    bstrafe = dsda_InputMouseBActive(dsda_input_strafe) ||
              dsda_InputJoyBActive(dsda_input_strafe);
  ...
  if (strafe) {
    ...
    true_delta =
        mousestrafe_carry + (double)mousex / movement_mousestrafedivisor;
...
dboolean G_Responder(event_t *ev) {
  if (gamestate == GS_LEVEL &&
      (HU_Responder(ev) || ST_Responder(ev) || AM_Responder(ev)))
    return true;
  ...
  if (dsda_InputActivated(dsda_input_spy) && netgame &&
  ...
  case ev_mousemotion: {
    ...
    value = dsda_FineSensitivity(mouseSensitivity_horiz) *
            AccelerateMouse(ev->data2);
...
void G_WorldDone(void) {
  ...
  if (gamemode == commercial && gamemission != pack_nerve) {
    ...
    gameaction =
        ga_victory; // cph - after ExM8 summary screen, show victory stuff
}
...
void(CheckSaveGame)(size_t size, const char *file, int line) {
  ...
  if (pos > prev_check)
    I_Error("CheckSaveGame at %s:%d called for insufficient buffer (%u < %u)",
  ...
  if (pos + size > savegamesize)
    save_p = (savebuffer =
                  realloc(savebuffer, savegamesize += (size + 1023) & ~1023)) +
             pos;
}
...
static void G_DoSaveGame(dboolean menu) {
  ...
  CheckSaveGame(dsda_GameOptionSize() + MIN_MAXPLAYERS + 14 +
...
void G_Compatibility(void) {
  static const struct {
    ...
    complevel_t opt; // level at which fix/change was made optional
      ...
      // comp_telefrag - monsters used to telefrag only on MAP30, now they do it
      // for spawners only
      ...
      // comp_pain - original Doom limits Pain Elementals from spawning too many
      // skulls
      {boom_compatibility, mbf_compatibility},
      // comp_skull - original Doom let skulls be spit through walls by Pain
      // Elementals
      {boom_compatibility, mbf_compatibility},
      ...
      // comp_respawn - objects which aren't on the map at game start respawn at
      // (0,0)
...
void G_InitNew(skill_t skill, int episode, int map) {
  ...
  // Disable all sanity checks if there are custom episode definitions. They do
...
void G_BeginRecording(void) {
  ...
  if (mbf_features) {
    ...
    *demo_p++ = consoleplayer;
    ...
    // FIXME } else if (compatibility_level >= boom_compatibility_compatibility)
    // { //e6y
...
const byte *G_ReadDemoHeaderEx(const byte *demo_p, size_t size,
  ...
  if (demover == 255) {
    ...
    if (strncmp((const char *)demo_p, prdemosig, 5) != 0) {
      I_Error("G_ReadDemoHeader: Extended demo format 255 found, but \"PR+UM\" "
              "string not found.");
    }
    ...
    for (i = 0; i < num_extensions; i++) {
      ...
      if (r_len == 8 && strncmp((const char *)demo_p, "UMAPINFO", 8) == 0) {
        ...
        I_Error(
            "G_ReadDemoHeader: Extended demo format extension unrecognized.");
      }
    ...
    if (using_umapinfo) {
      ...
      if (strncmp((const char *)demo_p, "MAP", 3) == 0) {
        ...
        for (i = 0; demo_p < string_end; i++) {
          ...
          if (cur < '0' || cur > '9') {
            if (cur != 0) {
              I_Error("G_ReadDemoHeader: Unable to determine map for UMAPINFO "
                      "demo.");
            }
            break;
          }
        ...
        for (i = 0; demo_p < string_end; i++) {
          ...
          if (cur < '0' || cur > '9') {
            ...
            I_Error(
                "G_ReadDemoHeader: Unable to determine map for UMAPINFO demo.");
          }
  ...
  // the only stuff related to it will be behind extension_version checks past
  // this point
  ...
  if (!((demover >= 0 && demover <= 4) || (demover >= 104 && demover <= 111) ||
        (demover >= 200 && demover <= 214) || (demover == 221))) {
  ...
  if (demover < 200) // Autodetect old demos
  {
    ...
    if ((skill = demover) >= 100) // For demos from versions >= 1.4
    {
      ...
      if (*(header_p + size - 1) == DEMOMARKER) {
        ...
        if ((size >= 8 && (size - 8) % 4 != 0 && !heretic) || (map > 9) ||
  ...
  } else // new versions of demos
  {
    ...
    // e6y: check for overrun
    if (CheckForOverrun(header_p, demo_p, size, dsda_GameOptionSize(),
                        failonerror))
      return NULL;
...
void G_ReadDemoContinueTiccmd(ticcmd_t *cmd) {
  ...
  if (gametic >= demo_tics_count || demo_continue_p > demobuffer + demolength ||
      dsda_InputActive(dsda_input_join_demo) ||
      dsda_InputJoyBActive(dsda_input_use)) {

...
byte filter_ditherMatrix[DITHER_DIM][DITHER_DIM] = {
    {0 * DMR, 14 * DMR, 3 * DMR, 13 * DMR},
    {11 * DMR, 5 * DMR, 8 * DMR, 6 * DMR},
    {12 * DMR, 2 * DMR, 15 * DMR, 1 * DMR},
...
void R_FilterInit(void) {
  ...
  //
  // #define getCode(b,f,h,d) ( (b == f)<<0 | (f == h)<<1 | (h == d)<<2 | (d ==
  // b)<<3 )

  // encode the scale2x conditionals into a lookup code
  for (i = 0; i < 16; i++) {
    //  E0 = D == B && B != F && D != H ? D : E; // 10-0 => 1000 or 1010 => 8 or
    //  A
    filter_roundedRowMap[0 * 16 + i] = (i == 0x8 || i == 0xA) ? 0 : 1;
    //  E1 = B == F && B != D && F != H ? F : E; // 0-01 => 0101 or 0001 => 5 or
    //  1
    filter_roundedRowMap[1 * 16 + i] = (i == 0x5 || i == 0x1) ? 2 : 1;
    //  E2 = D == H && D != B && H != F ? D : E; // 010- => 0101 or 0100 => 5 or
    //  4
    filter_roundedRowMap[2 * 16 + i] = (i == 0x4 || i == 0x5) ? 0 : 1;
    //  E3 = H == F && D != H && B != F ? F : E; // -010 => 1010 or 0010 => A or
    //  2
...
#define getCode(b, f, h, d)                                                    \
  ((b == f) << 0 | (f == h) << 1 | (h == d) << 2 | (d == b) << 3)

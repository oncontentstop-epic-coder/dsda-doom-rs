...
void gld_InitPalettedTextures(void) {
  ...
  for (i = 0; i < 256; i++) {
    pal[i] = (playpal[i * 3 + 0] << 16) | (playpal[i * 3 + 1] << 8) |
             playpal[i * 3 + 2];
    gld_palmap[i] = i;
  }
...
GLTexture *gld_RegisterTexture(int texture_num, dboolean mipmap,
  ...
  if (gltexture->textype == GLDT_UNREGISTERED) {
    ...
    gltexture->scalexfac =
        (float)gltexture->width / (float)gltexture->tex_width;
    gltexture->scaleyfac =
        (float)gltexture->height / (float)gltexture->tex_height;

    gltexture->buffer_size =
        gltexture->buffer_width * gltexture->buffer_height * 4;
...
static void gld_RecolorMipLevels(byte *data) {
  ...
  if (gl_color_mip_levels) {
    ...
    for (miplevel = 1; miplevel < 16; miplevel++) {
      ...
      glTexImage2D(GL_TEXTURE_2D, miplevel, gl_tex_format, w, h, 0, GL_RGBA,
...
int gld_BuildTexture(GLTexture *gltexture, void *data, dboolean readonly,
...
#endif // USE_GLU_MIPMAP
  {
    ...
    if ((width != tex_width) || (height != tex_height)) {
      ...
      gluScaleImage(GL_RGBA, width, height, GL_UNSIGNED_BYTE, data, tex_width,
...
GLTexture *gld_RegisterPatch(int lump, int cm, dboolean is_sprite) {
  ...
  if (gltexture->textype == GLDT_UNREGISTERED) {
    ...
    gltexture->width = MIN(gltexture->realtexwidth, gltexture->tex_width);
    gltexture->height = MIN(gltexture->realtexheight, gltexture->tex_height);
    gltexture->buffer_width =
        MAX(gltexture->realtexwidth, gltexture->tex_width);
  ...
  for (i = numsectors; --i >= 0;) {
    ...
    for (k = 0; k < 2; k++) {
      if (flatanims[k] && !flatanims[k]->istexture) {
        for (j = flatanims[k]->basepic;
             j < flatanims[k]->basepic + flatanims[k]->numpics; j++)
          hitlist[j] = 1;
      }
    }
  }
    ...
    if (hitlist[i]) {
      ...
      while (--j >= 0) {
        ...
        do {
          ...
          gltexture =
              gld_RegisterPatch(firstspritelump + sflump[k], CR_LIMIT, true);
          if (gltexture) {

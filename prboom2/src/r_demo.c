...
void R_SmoothPlaying_Add(int delta) {
  if (demo_smoothturns && demoplayback) {
    ...
    smooth_playing_index =
        (smooth_playing_index + 1) % (demo_smoothturnsfactor);
    smooth_playing_sum += delta;
    smooth_playing_angle +=
        (int)(smooth_playing_sum / (demo_smoothturnsfactor));
  }
}
...
void R_DemoEx_WriteMLook(angle_t pitch) {
  ...
  if (mlook_lump.tick >= mlook_lump.maxtick) {
    ...
    mlook_lump.data = realloc(mlook_lump.data,
                              mlook_lump.maxtick * sizeof(mlook_lump.data[0]));
...
static void R_DemoEx_GetParams(const byte *pwad_p, waddata_t *waddata) {
  ...
  if (params) {
    ...
    M_ParseCmdLine(str, params, ((char *)params) + sizeof(char *) * paramscount,
                   &paramscount, &i);
      ...
      if (!CheckWadBufIntegrity(demoex_p, size)) {
        ...
        for (i = 0; i < numwadfiles; i++) {
          if (wadfiles[i].src == source_auto_load ||
              wadfiles[i].src == source_pre || wadfiles[i].src == source_lmp) {
...
int CheckAutoDemo(void) {
  ...
  if (M_CheckParm("-auto"))
    ...
    I_Error("Cannot process -auto - " PACKAGE_NAME
            " was compiled without LIBPCRE support");
#else
  {
    int i;
    waddata_t waddata;
    ...
    for (i = 0; (size_t)i < numwadfiles; i++) {
      if (wadfiles[i].src == source_lmp) {
        ...
        numwadfiles_required =
            DemoNameToWadData(wadfiles[i].name, &waddata, &patterndata);

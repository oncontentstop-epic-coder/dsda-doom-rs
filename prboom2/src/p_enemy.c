...
static fixed_t P_AvoidDropoff(mobj_t *actor) {
  ...
  int xl =
      P_GetSafeBlockX((tmbbox[BOXLEFT] = actor->x - actor->radius) - bmaporgx);
  int bx, by;
...
static void P_NewChaseDir(mobj_t *actor) {
  ...
  if (mbf_features) {
        ...
        P_AvoidDropoff(actor)) /* Move away from dropoff */
    {
      ...
      } else if (target->health > 0 && (actor->flags ^ target->flags) &
                                           MF_FRIEND) { // Live enemy target
             ...
             (target->player && dist < MELEERANGE * 3 &&
              (target->player->readyweapon == wp_fist ||
               target->player->readyweapon ==
                   wp_chainsaw)))) { // Back away from melee attacker
...
static dboolean P_LookForPlayers(mobj_t *actor, dboolean allaround) {
  ...
  if (heretic)
    return Heretic_P_LookForPlayers(actor, allaround);
...
static dboolean P_LookForTargets(mobj_t *actor, int allaround) {
  return actor->flags & MF_FRIEND ? P_LookForMonsters(actor, allaround) ||
...
void A_Chase(mobj_t *actor) {
  ...
  if (heretic &&
      gameskill == sk_nightmare) { // Monsters move faster in nightmare mode
...
void A_SPosAttack(mobj_t *actor) {
  ...
  for (i = 0; i < 3;
       i++) { // killough 5/5/98: remove dependence on order of evaluation:
...
void A_BossDeath(mobj_t *mo) {
  ...
  if (heretic)
    return Heretic_A_BossDeath(mo);

  // numbossactions == 0 means to use the defaults.
  // numbossactions == -1 means to do nothing.
  // positive values mean to check the list of boss actions and run all that
  // apply.
  ...
  if (gamemode == commercial) {
    ...
    if (comp[comp_666] && gameepisode < 4) {
      ...
      // plutonia.exe and tnt.exe There is no more desync on
      // doom.wad\episode3.lmp
...
void P_SpawnBrainTargets(void) // killough 3/26/98: renamed old function
{
  ...
  for (thinker = thinkercap.next; thinker != &thinkercap;
    ...
    if (thinker->function == P_MobjThinker) {
      ...
      if (m->type ==
          MT_BOSSTARGET) { // killough 2/7/98: remove limit on icon landings:
...
void A_BrainExplode(
    mobj_t *mo) { // killough 5/5/98: remove dependence on order of evaluation:
...
void A_KnightAttack(mobj_t *actor) {
  ...
  // Throw axe
  S_StartSound(actor, actor->info->attacksound);
  if (actor->type == HERETIC_MT_KNIGHTGHOST ||
      P_Random(pr_heretic) < 40) { // Red axe
...
void A_Srcr2Decide(mobj_t *actor) {
  ...
  if (P_Random(pr_heretic) <
      chance[actor->health / (actor->info->spawnhealth / 8)]) {
...
void A_Srcr2Attack(mobj_t *actor) {
  ...
  if (P_Random(pr_heretic) < chance) { // Wizard spawners
...
void A_SorZap(mobj_t *actor) { S_StartSound(NULL, heretic_sfx_sorzap); }

void A_SorRise(mobj_t *actor) { S_StartSound(NULL, heretic_sfx_sorrise); }

void A_SorDSph(mobj_t *actor) { S_StartSound(NULL, heretic_sfx_sordsph); }

void A_SorDExp(mobj_t *actor) { S_StartSound(NULL, heretic_sfx_sordexp); }

void A_SorDBon(mobj_t *actor) { S_StartSound(NULL, heretic_sfx_sordbon); }

void A_SorSightSnd(mobj_t *actor) { S_StartSound(NULL, heretic_sfx_sorsit); }
...
void A_MinotaurDecide(mobj_t *actor) {
  ...
  if (target->z + target->height > actor->z &&
      ...
      P_Random(pr_heretic) < 150) { // Charge attack
    ...
    actor->special1.i = 35 / 2; // Charge duration
             ...
             P_Random(pr_heretic) < 220) { // Floor fire attack
...
void Heretic_A_BossDeath(mobj_t *actor) {
  ...
  thinker_t *think;
  line_t dummyLine;
  static mobjtype_t bossType[6] = {HERETIC_MT_HEAD,      HERETIC_MT_MINOTAUR,
                                   HERETIC_MT_SORCERER2, HERETIC_MT_HEAD,
                                   HERETIC_MT_MINOTAUR,  -1};
...
dboolean Heretic_P_LookForMonsters(mobj_t *actor) {
  ...
  for (think = thinkercap.next; think != &thinkercap; think = think->next) {
    ...
    if (P_Random(pr_heretic) < 16) { // Skip
      continue;
    }
...
dboolean Heretic_P_LookForPlayers(mobj_t *actor, dboolean allaround) {
  ...
  for (;; actor->lastlook = (actor->lastlook + 1) & 3) {
    ...
    if (player->mo->flags & MF_SHADOW) { // Player is invisible
      ...
      if (P_Random(pr_heretic) <
          225) { // Player isn't sneaking, but still didn't detect
        return (false);
      }
    }
    P_SetTarget(&actor->target, player->mo);

...
angle_t R_PointToAngleEx2(fixed_t x1, fixed_t y1, fixed_t x, fixed_t y) {
  ...
  if (x_viewx < INT_MIN || x_viewx > INT_MAX || y_viewy < INT_MIN ||
      y_viewy > INT_MAX) {
...
angle_t R_PointToPseudoAngle(fixed_t x, fixed_t y) {
  // Note: float won't work here as it's less precise than the BAM values being
...
static void R_InitTextureMapping(void) {
  ...
  if (wide_centerx != centerx) { // wide_centerx is what centerx would be if the
                                 // display was not widescreen
...
static void R_InitLightTables(void) {
  ...
  for (i = 0; i < LIGHTLEVELS; i++) {
    ...
    // darker.
    int j, startmap = ((LIGHTLEVELS - LIGHTBRIGHT - i) * 2) * NUMCOLORMAPS /
                      LIGHTLEVELS;
...
static void InitStretchParam(stretch_param_t *offsets, int stretch,
  ...
  if (flags == VPT_ALIGN_LEFT || flags == VPT_ALIGN_LEFT_BOTTOM ||
      flags == VPT_ALIGN_LEFT_TOP) {
    offsets->deltax1 = 0;
    offsets->deltax2 = 0;
  }
  ...
  if (flags == VPT_ALIGN_TOP || flags == VPT_ALIGN_LEFT_TOP ||
      flags == VPT_ALIGN_RIGHT_TOP) {
    offsets->deltay1 = 0;
  }
...
void R_SetupViewScaling(void) {
  ...
  // they was only calculating the top-left corner of the scaled pixels.
  ...
  // coords were. Thusly, all rectangles should be converted to unscaled x1, y1,
  // x2, y2 coords, scaled, and then converted back to x, y, w, h
  //
  // e6y: wide-res
  ...
  GenLookup(video_stretch.x1lookup, video_stretch.x2lookup, video_stretch.width,
...
void R_SetupViewport(void) {
  ...
  viewport[1] =
      SCREENHEIGHT - (height + viewwindowy - ((height - viewheight) / 2));

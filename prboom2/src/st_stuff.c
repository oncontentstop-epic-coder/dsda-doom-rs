...
// face background
static patchnum_t
    faceback; // CPhipps - single background, translated for different players
...
static void ST_updateFaceWidget(void) {
  ...
  if (priority < 8) {
    if (plyr->damagecount && plyr->attacker && plyr->attacker != plyr->mo) {
      ...
      if ((comp[comp_ouchface] ? (plyr->health - st_oldhealth)
...
static void ST_doPaletteStuff(void) {
  ...
  } else if (dsda_BonusPalette() && plyr->bonuscount) {
    palette = (plyr->bonuscount + 7) >> 3;
    if (palette >= NUMBONUSPALS)
      palette = NUMBONUSPALS - 1;
    palette += STARTBONUSPALS;
  } else if (dsda_PowerPalette() && (plyr->powers[pw_ironfeet] > 4 * 32 ||
  ...
  if (dsda_PowerPalette() && gamemission == chex && palette >= STARTREDPALS &&
...
void M_ChangeApplyPalette(void) { ST_doPaletteStuff(); }
...
void ST_Drawer(dboolean statusbaron, dboolean refresh, dboolean fullmenu) {
  if (heretic)
    return SB_Drawer(statusbaron, refresh, fullmenu);
...
static void ST_loadGraphics(dboolean doload) {
  ...
  // unlocks it. var is referenced exactly once in either case, so ++ in arg
  // works

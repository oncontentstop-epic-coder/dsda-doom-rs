...
void gld_ProgressUpdate(const char *text, int progress, int total) {
  ...
  if ((text) && (strlen(text) > 0) &&
      strcmp((last_text[0] ? last_text : ""), text)) {
    const char *s;
...
typedef struct _DDRAW_H_DDPIXELFORMAT {
  ...
  union {
    ...
    unsigned long dwZBufferBitDepth;   // how many total bits/pixel in z buffer
    ...
    unsigned long
        dwPrivateFormatBitCount; // Bits per pixel of private driver formats.
  ...
  union {
    ...
    unsigned long dwStencilBitDepth;  // how many stencil bits (note:
                                      // dwZBufferBitDepth-dwStencilBitDepth is
                                      // total Z-only bits)
  ...
  union {
    ...
    struct {
      unsigned short wFlipMSTypes; // Multisample methods supported via flip for
                                   // this D3DFORMAT
      unsigned short wBltMSTypes;  // Multisample methods supported via blt for
                                   // this D3DFORMAT
    } MultiSampleCaps;
...
typedef struct _DDRAW_H_DDSURFACEDESC2 {
  ...
  union {
    ...
    unsigned long
        dwLinearSize; // Formless late-allocated optimized surface size
  } u1;
  union {
  ...
  union {
    unsigned long dwMipMapCount; // number of mip-map levels requestde
                                 // dwZBufferBitDepth removed, use
                                 // ddpfPixelFormat one instead
    unsigned long
        dwRefreshRate; // refresh rate (used when display mode is described)
...
GLGenericImage *ReadDDSFile(const char *filename, int *bufsize,
                            int *numMipmaps) {
          ...
          *bufsize = ddsd.u2.dwMipMapCount > 1 ? ddsd.u1.dwLinearSize * factor
          ...
          if (fread(genericImage->pixels, 1, *bufsize, fp) > 0) {
            ...
            genericImage->components =
                (ddsd.u4.ddpfPixelFormat.dwFourCC == DDRAW_H_FOURCC_DXT1) ? 3
                                                                          : 4;
  ...
  if (GLEXT_glCompressedTexImage2DARB) {
    ...
    if (ddsimage) {
      ...
      if (tex_width == ddsimage->width && tex_height == ddsimage->height) {
        ...
        offset = 0;
        blockSize =
            (ddsimage->format == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) ? 8 : 16;
        ...
        for (i = 0; i < (numMipmaps ? numMipmaps : 1) &&
          ...
          GLEXT_glCompressedTexImage2DARB(GL_TEXTURE_2D, i, ddsimage->format,
                                          ddsimage->width, ddsimage->height, 0,
                                          size, ddsimage->pixels + offset);
...
static int gld_HiRes_LoadFromCache(GLTexture *gltexture, GLuint *texid,
  ...
  if (cachefp) {
    if (fread(&tex_width, sizeof(tex_width), 1, cachefp) == 1 &&
        ...
        fread(&cache_stat.st_mtime, sizeof(cache_stat.st_mtime), 1, cachefp) ==
            1) {
      if (cache_stat.st_mtime == tex_stat.st_mtime) {
        ...
        if (tex_buffer) {
          if (fread(tex_buffer, tex_buffer_size, 1, cachefp) == 1) {
            gld_HiRes_Bind(gltexture, texid);
            gld_BuildTexture(gltexture, tex_buffer, false, tex_width,
                             tex_height);
    ...
    if (buf) {
      ...
      if (cachefp) {
        result = (fwrite(&w, sizeof(w), 1, cachefp) == 1) &&
                 ...
                 (fwrite(&tex_stat.st_mtime, sizeof(tex_stat.st_mtime), 1,
                         cachefp) == 1) &&
          ...
          if (lump != -1) {
            ...
            if (!surf_tmp) {
              ...
              if (surf) {
                ...
                gld_HiRes_Bind(gltexture, texid);
                gld_BuildTexture(gltexture, surf->pixels, true, surf->w,
                                 surf->h);
...
int gld_PrecacheGUIPatches(void) {
  static const char *staticpatches[] = {
      ...
      "M_THERMO", "M_THERMR", "M_ULTRA", "M_EPI4",

      // prboom's M_*
      "M_ACCEL", "M_AUTO", "M_BUTT1", "M_BUTT2", "M_CHAT", "M_COLORS",
      "M_COMPAT", "M_COMPAT", "M_ENEM", "M_GENERL", "M_HORSEN", "M_KEYBND",

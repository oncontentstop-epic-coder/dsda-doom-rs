...
static int P_CheckForZDoomUncompressedNodes(int lumpnum, int gl_lumpnum) {
  ...
  if (result) {
    ...
    if (result) {
      lprintf(LO_INFO, "P_CheckForZDoomUncompressedNodes: compressed ZDoom "
                       "nodes are detected\n");
...
static void P_GetNodesVersion(int lumpnum, int gl_lumpnum) {
  ...
  if ((gl_lumpnum > lumpnum) && (mbf21 || forceOldBsp == false) &&
...
static void P_LoadSegs(int lump) {
  ...
  data =
      (const mapseg_t *)W_CacheLumpNum(lump); // cph - wad lump handling updated
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if (v1 >= numvertexes || v2 >= numvertexes) {
      char str[200] = "P_LoadSegs: compatibility loss - seg %d references a "
                      "non-existent vertex %d\n";
...
static void P_LoadSegs_V4(int lump) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if (side != 0 && side != 1) {
      lprintf(LO_WARN,
              "P_LoadSegs_V4: seg %d contains wrong side index %d. Replaced "
              "with 1.\n",
              i, side);
      side = 1;
    }
    ...
    // e6y: check for wrong indexes
    if ((unsigned)ldef->sidenum[side] >= (unsigned)numsides) {
      I_Error("P_LoadSegs_V4: linedef %d for seg %d references a non-existent "
              "sidedef %d",
              linedef, i, (unsigned)ldef->sidenum[side]);
    }
    ...
    // http://www.doomworld.com/idgames/index.php?id=12647
    if (v1 >= numvertexes || v2 >= numvertexes) {
      char str[200] = "P_LoadSegs_V4: compatibility loss - seg %d references a "
                      "non-existent vertex %d\n";
...
static void P_LoadZSegs(const byte *data) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if (side != 0 && side != 1) {
      lprintf(LO_WARN,
              "P_LoadZSegs: seg %d contains wrong side index %d. Replaced with "
              "1.\n",
              i, side);
      side = 1;
    }
    ...
    // e6y: check for wrong indexes
    if ((unsigned)ldef->sidenum[side] >= (unsigned)numsides) {
      I_Error("P_LoadZSegs: linedef %d for seg %d references a non-existent "
              "sidedef %d",
              linedef, i, (unsigned)ldef->sidenum[side]);
    }
...
static void P_LoadZNodes(int lump, int glnodes, int compressed) {
  ...
  if (compressed == ZDOOM_ZNOD_NODES) {
    ...
    if (inflateEnd(zstream) != Z_OK)
      I_Error(
          "P_LoadZNodes: Error during ZDoom nodes decompression shut-down!");
    ...
    if (!vert) // don't interesect vertical lines with columns
    {
      for (j = 0; j < ncols; j++) {
        ...
        if (yp == 0) // intersection at a corner
        {
          ...
          AddBlockLine(blocklists, blockcount, blockdone, ncols * yb + j - 1,
                       i);
      }
    }
    ...
    if (!horiz) {
      for (j = 0; j < nrows; j++) {
        ...
        if (xp == 0) // intersection at a corner
        {
          ...
          AddBlockLine(blocklists, blockcount, blockdone, ncols * (j - 1) + xb,
                       i);
      }
    }
  }
...
static void P_LoadBlockMap(int lump) {
  ...
  if (M_CheckParm("-blockmap") || W_LumpLength(lump) < 8 ||
    ...
    if (!P_VerifyBlockMap(count)) {
      ...
      lprintf(LO_INFO, "P_LoadBlockMap: use \"-blockmap\" command line switch "
                       "for rebuilding\n");
    }
  }
...
static void R_CalcSegsLength(void) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    li->pangle =
        R_PointToAngleEx2(li->v1->px, li->v1->py, li->v2->px, li->v2->py);
  }
}
...
void P_CheckLevelWadStructure(const char *mapname) {
  ...
  for (i = ML_THINGS + 1; i <= ML_SECTORS; i++) {
    if (!P_CheckLumpsForSameSource(lumpnum, lumpnum + i)) {
      I_Error("P_SetupLevel: Level wad structure is incomplete. There is no %s "
              "lump.",
              ml_labels[i]);
    }
  }

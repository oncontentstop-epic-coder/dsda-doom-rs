...
// R_ClipWallSegment
//
// Replaces the old R_Clip*WallSegment functions. It draws bits of walls in
// those columns which aren't solid, and updates the solidcol[] array
// appropriately
...
static void R_RecalcLineFlags(line_t *linedef) {
  ...
  if (linedef->flags & ML_TWOSIDED) {
    ...
    if ((c = frontsector->floorheight - backsector->floorheight) > 0 &&
        (textureheight[texturetranslation[curline->sidedef->bottomtexture]] >
         c))
...
static void R_Subsector(int num) {
...
#ifdef GL_DOOM
  // dmooter 1/16/2017 Move from being declared next to its use several lines
  // lower. Needs to remain in scope to the end of the function so its stack
  // memory is recycled, compiler optimizations will make the floorplane pointer
  // a dangling pointer when passed into gld_AddPlane().
...
#ifdef GL_DOOM
  if (V_GetMode() != VID_MODEGL || !gl_use_stencil ||
  ...
  if (V_GetMode() == VID_MODEGL) {
    ...
    if (!gl_use_stencil && frontsector == sub->sector) {
      ...
      // if the sector has bottomtextures, then the floorheight will be set to
      // the highest surounding floorheight

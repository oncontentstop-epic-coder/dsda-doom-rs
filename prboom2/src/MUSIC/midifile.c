...
static dboolean ReadTrack(midi_track_t *track, midimem_t *mf) {
  ...
  for (;;) {
    ...
    */
    if (track->num_events ==
        track->num_event_mem) { // depending on the state of the heap and the
                                // malloc implementation, realloc()
...
static double
compute_spmc_normal(unsigned mpq, unsigned tempo,
                    unsigned sndrate) { // returns samples per midi clock
...
static double
compute_spmc_smpte(unsigned smpte_fps, unsigned mpf,
                   unsigned sndrate) { // returns samples per midi clock
  ...
  while (1) {
    ...
    if (ret->tracks->num_events == ret->tracks->num_event_mem) {
      ...
      ret->tracks->events = (midi_event_t *)realloc(
          ret->tracks->events,
          sizeof(midi_event_t) * ret->tracks->num_event_mem);
    }
    ...
    if (oldev->event_type == MIDI_EVENT_META) {
      if (oldev->data.meta.type ==
          MIDI_META_SET_TEMPO) { // adjust future tempo scaling
      ...
      if (oldev->data.meta.type ==
          MIDI_META_END_OF_TRACK) { // reproduce event and break
        nextev->event_type = MIDI_EVENT_META;

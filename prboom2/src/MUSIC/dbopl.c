...
/*
  ...
  Enabling the opl3 bit will switch the emulator to stereo opl3 output instead
  of regular mono opl2 Except for the table generation it's all integer math Can
  choose different types of generators, using muls and bigger tables, try
  different ones for slower platforms The generation was based on the MAME
  implementation but tried to have it use less memory and be faster in general
  MAME uses much bigger envelope tables and this will be the biggest cause of it
  sounding different at times

  //TODO Don't delay first operator 1 sample in opl3 mode
  //TODO Maybe not use class method pointers but a regular function pointers
  with operator as first parameter
  //TODO Fix panning for the Percussion channels, would any opl3 player use it
  and actually really change it though?
  //TODO Check if having the same accuracy in all frequency multipliers sounds
  better or not
...
#define WAVE_BITS 10
#else
// Need some extra bits at the top to have room for octaves and frequency
// multiplier We support to 8 times lower rate 128 * 15 * 8 = 15350, 2^13.9, so
// need 14 bits
#define WAVE_BITS 14
#endif
...
static const Bit8u AttackSamplesTable[13] = {69, 55, 46, 40, 35, 29, 23,
                                             20, 19, 15, 11, 10, 9};
// On a real opl these values take 8 samples to reach and are based upon larger
// tables
...
// Generate a table index and table shift value using input value from a
// selected rate
...
#if (DBOPL_WAVE == WAVE_HANDLER)
/*
  Generate the different waveforms out of the sine/exponetial table using
  handlers
*/
...
static const WaveHandler WaveHandlerTable[8] = {WaveForm0, WaveForm1, WaveForm2,
...
static const VolumeHandler VolumeHandlerTable[5] = {
    &Operator__TemplateVolumeOFF,     &Operator__TemplateVolumeRELEASE,
    &Operator__TemplateVolumeSUSTAIN, &Operator__TemplateVolumeDECAY,
...
static inline void Operator__Prepare(Operator *self, const Chip *chip) {
  self->currentLevel =
      self->totalLevel + (chip->tremoloValue & self->tremoloMask);
...
static inline void Channel__GeneratePercussion(Channel *self, Chip *chip,
  ...
  phaseBit = (((c2 & 0x88) ^ ((c2 << 5) & 0x80)) | ((c5 ^ (c5 << 2)) & 0x20))
                 ? 0x02
                 : 0x00;
...
Channel *Channel__BlockTemplate(Channel *self, Chip *chip, Bit32u samples,
  ...
  for (i = 0; i < samples; i++) {
    ...
    // Do unsigned shift so we can shift out all bits but still stay in 10 bit
    // range otherwise
...
static inline Bit32u Chip__ForwardLFO(Chip *self, Bit32u samples) {
  ...
  self->vibratoShift =
      (VibratoTable[self->vibratoIndex >> 2] & 7) + self->vibratoStrength;
  self->tremoloValue =
      TremoloTable[self->tremoloIndex] >> self->tremoloStrength;
...
void DBOPL_InitTables(void) {
  ...
  for (i = 0; i < 384; i++) {
    ...
    double val =
        (0.5 + (pow(2.0, -1.0 + (255 - s) * (1.0 / 256))) * (1 << MUL_SH));

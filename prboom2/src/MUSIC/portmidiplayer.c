...
// the last time render was called to this time.  portmidi latency instruction
// must be larger than that window so the messages appear in the future.
// ~46-47ms is the nominal length if i_sound.c gets its way
...
static void pm_shutdown(void) {
  if (pm_stream) {
    ...
    since timestamps are slightly in the future, it's very possible to have some
    messages still in the windows midi queue when Pm_Close is called.  this is
    normally no problem, but if one so happens to dequeue and call
    winmm_streamout_callback at the exact right moment...

    fix: at this point, we've stopped generating midi messages.  sleep for more
    than DRIVER_LATENCY to ensure all messages are flushed.
...
static void writesysex(unsigned long when, int etype, unsigned char *data,
                       int len) {

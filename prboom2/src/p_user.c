...
void P_MovePlayer(player_t *player) {
  ...
  if ((player->mo->flags & MF_FLY) && player == &players[consoleplayer] &&
      upmove != 0) {
    mo->momz = upmove << 8;
  }
      ...
      (cmd->forwardmove | cmd->sidemove)) // killough 10/98
  {
    if (onground || mo->flags & MF_BOUNCES ||
        (mo->flags & MF_FLY)) // killough 8/9/98
    {
...
void P_PlayerThink(player_t *player) {
  ...
  if (player->powers[pw_weaponlevel2]) {
    if (!--player->powers[pw_weaponlevel2]) {
      if ((player->readyweapon == wp_phoenixrod) &&
          ...
          (player->psprites[ps_weapon].state != &states[HERETIC_S_PHOENIXUP])) {
  ...
  if (!heretic)
    player->fixedcolormap =
        dsda_PowerPalette() && (player->powers[pw_invulnerability] > 4 * 32 ||
...
void P_ChickenPlayerThink(player_t *player) {
  ...
  pmo = player->mo;
  if (!(pmo->momx + pmo->momy) &&
      P_Random(pr_heretic) < 160) { // Twitch view angle
    pmo->angle += P_SubRandom() << 19;
  }
  if ((pmo->z <= pmo->floorz) &&
      (P_Random(pr_heretic) < 32)) { // Jump and noise
    pmo->momz += FRACUNIT;
    P_SetMobjState(pmo, HERETIC_S_CHICPLAY_PAIN);
    return;
  }
  if (P_Random(pr_heretic) < 48) { // Just noise

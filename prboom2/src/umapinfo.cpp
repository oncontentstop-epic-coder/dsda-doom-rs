...
static char *ParseMultiString(Scanner &scanner, int error) {
  ...
  do {
    ...
    if (build == NULL)
      ...
      strcat(build,
             scanner.string); // Concatenate the new line onto the existing text
    }
...
int ParseUMapInfo(const unsigned char *buffer, size_t length,
  ...
  while (scanner.TokensLeft()) {
    ...
    if (i == Maps.mapcount) {
      Maps.mapcount++;
      Maps.maps =
          (MapEntry *)realloc(Maps.maps, sizeof(MapEntry) * Maps.mapcount);
...
MapProperty *FindProperty(MapEntry *map, const char *name) { return NULL; }

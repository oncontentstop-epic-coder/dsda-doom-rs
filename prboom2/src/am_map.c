...
#define F_PANINC                                                               \
  (dsda_InputActive(dsda_input_speed) ? map_scroll_speed * 2 : map_scroll_speed)
...
static void AM_addMark(void) {
  ...
  if (markpointnum >= markpointnum_max)
    markpoints =
        realloc(markpoints, (markpointnum_max =
...
void AM_SetPosition(void) {
  if (automapmode & am_overlay) {
    ...
    f_h = SCREENHEIGHT -
          ST_SCALED_HEIGHT; // to allow runtime setting of width/height
  }
}
...
void AM_rotatePoint(mpoint_t *p) {
  ...
  tmpx = FixedMul(p->x, am_frame.cos) - FixedMul(p->y, am_frame.sin) +
         am_frame.centerx;
  p->y = FixedMul(p->x, am_frame.sin) + FixedMul(p->y, am_frame.cos) +
         am_frame.centery;
  p->x = tmpx;
}
...
#define DOOUTCODE(oc, mx, my)                                                  \
  ...
  while (outcode1 | outcode2) {
    ...
    if (outside & TOP) {
      ...
      dx = fl->b.x - fl->a.x;
      tmp.x =
          fl->a.x + (fixed_t)(((int_64_t)dx * (fl->a.y - (f_y + f_h))) / dy);
      tmp.y = f_y + f_h - 1;
      if (am_frame.precise) {
        dy_f = fl->a.fy - fl->b.fy;
        dx_f = fl->b.fx - fl->a.fx;
      ...
      dx = fl->b.x - fl->a.x;
      tmp.y =
          fl->a.y + (fixed_t)(((int_64_t)dy * (f_x + f_w - 1 - fl->a.x)) / dx);
      tmp.x = f_x + f_w - 1;
      if (am_frame.precise) {
...
static void AM_drawGrid(int color) {
  ...
  minlen =
      M_DoubleToInt(sqrt((float)m_w * (float)m_w + (float)m_h * (float)m_h));
  extx = (minlen - m_w) / 2;
  exty = (minlen - m_h) / 2;
  ...
  for (i = 0; i < numlines; i++) {
    ...
    if (ddt_cheating || (lines[i].flags & ML_MAPPED)) {
        ...
        continue;
      {
        ...
        if (((*mapcolor_bdor_p) || (*mapcolor_ydor_p) || (*mapcolor_rdor_p)) &&
            ...
            (amd = AM_DoorColor(lines[i].special)) != -1) {
          {
            ...
            case 1:
              /*bluekey*/
              AM_drawMline(&l, (*mapcolor_bdor_p) ? (*mapcolor_bdor_p)
                                                  : (*mapcolor_cchg_p));
              continue;
            case 2:
              /*yellowkey*/
              AM_drawMline(&l, (*mapcolor_ydor_p) ? (*mapcolor_ydor_p)
                                                  : (*mapcolor_cchg_p));
              continue;
            case 0:
              /*redkey*/
              AM_drawMline(&l, (*mapcolor_rdor_p) ? (*mapcolor_rdor_p)
                                                  : (*mapcolor_cchg_p));
              continue;
            case 3:
              /*any or all*/
              AM_drawMline(&l, (*mapcolor_clsd_p) ? (*mapcolor_clsd_p)
                                                  : (*mapcolor_cchg_p));
              continue;
            }
          }
      ...
      } else                                    /* now for 2S lines */
      {
        ...
        if ((*mapcolor_tele_p) && !(lines[i].flags & ML_SECRET) &&
            (lines[i].special == 39 ||
             (!heretic && (lines[i].special == 97 || lines[i].special == 125 ||
                           lines[i].special == 126)))) { // teleporters
          AM_drawMline(&l, (*mapcolor_tele_p));
        } else if (lines[i].flags & ML_SECRET) // secret door
        {
          AM_drawMline(&l, (*mapcolor_wall_p)); // wall color
        } else if ((*mapcolor_clsd_p) &&
          ...
          AM_drawMline(&l,
                       (*mapcolor_flat_p)); // 2S lines that appear only in IDDT
        }
      }
...
static void AM_DrawNiceThings(void) {
  ...
  // marked locations on the automap
  {
    ...
    for (i = 0; i < markpointnum;
         i++) // killough 2/22/98: remove automap mark limit
    {
      if (markpoints[i].x != -1) {
        mpoint_t p;
      ...
      while (t) // for all things in that sector
      {
        ...
        if ((*mapcolor_rkey_p) || (*mapcolor_ykey_p) || (*mapcolor_bkey_p)) {
          ...
          if (heretic) {
            ...
            case 73: // jff  red key
              color = (*mapcolor_rkey_p) != -1 ? (*mapcolor_rkey_p)
                                               : (*mapcolor_sprt_p);
              break;
            case 80: // jff yellow key
              color = (*mapcolor_ykey_p) != -1 ? (*mapcolor_ykey_p)
                                               : (*mapcolor_sprt_p);
              break;
            case 79: // jff blue key
              color = (*mapcolor_bkey_p) != -1 ? (*mapcolor_bkey_p)
                                               : (*mapcolor_sprt_p);
              break;
            }
          } else {
          ...
          if (color != -1) {
            AM_drawLineCharacter(cross_mark, NUMCROSSMARKLINES, scale, t->angle,
                                 color, p.x, p.y);
            t = t->snext;
            continue;
          }
        ...
        AM_drawLineCharacter(
            thintriangle_guy, NUMTHINTRIANGLEGUYLINES, scale, angle,
...
static void AM_drawMarks(void) {
       ...
       i++) // killough 2/22/98: remove automap mark limit
  {
    if (markpoints[i].x != -1) {
      ...
      if (am_frame.precise) {
        p.fx = CXMTOF_F(p.fx) -
               (float)markpoints[i].w * SCREENWIDTH / 320.0f / 2.0f;
        p.fy = CYMTOF_F(p.fy) -
               (float)markpoints[i].h * SCREENHEIGHT / 200.0f / 2.0f;
      }

      if (V_GetMode() == VID_MODEGL
              ? p.y < f_y + f_h &&
                    p.y + markpoints[i].h * SCREENHEIGHT / 200 >= f_y
              : p.y < f_y + f_h && p.y >= f_y) {
        w = 0;
        for (k = 0; k < (int)strlen(markpoints[i].label); k++) {
          ...
          if (p.x < f_x + f_w &&
            ...
            if (am_frame.precise) {
              V_DrawNamePatchPrecise((float)p.fx * 320.0f / SCREENWIDTH,
                                     (float)p.fy * 200.0f / SCREENHEIGHT, FB,
                                     namebuf, CR_DEFAULT,
                                     VPT_ALIGN_WIDE | VPT_STRETCH);
            } else {
              V_DrawNamePatch(p.x * 320 / SCREENWIDTH, p.y * 200 / SCREENHEIGHT,
...
void AM_ResetIDDTcheat(void) { ddt_cheating = 0; }

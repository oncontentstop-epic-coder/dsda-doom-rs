...
void dsda_InitSettings(void) { dsda_ChangeStrictMode(); }
...
void dsda_SetTas(void) { dsda_tas = true; }
...
dboolean dsda_CycleGhostColors(void) { return dsda_cycle_ghost_colors; }

dboolean dsda_AlwaysSR50(void) {
  return movement_strafe50 && !dsda_StrictMode();
}

dboolean dsda_HideHorns(void) { return dsda_hide_horns; }

dboolean dsda_ExHud(void) { return dsda_exhud; }
...
dboolean dsda_ShowHealthBars(void) { return health_bar && !dsda_StrictMode(); }

dboolean dsda_WipeAtFullSpeed(void) { return dsda_wipe_at_full_speed; }
...
void dsda_SkipNextWipe(void) { dsda_skip_next_wipe = 1; }

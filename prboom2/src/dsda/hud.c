...
static void dsda_UpdateExHud(void) {
  ...
  // Max totals - from hu_stuff.c
  {
    ...
    killcolor = (fullkillcount >= max_kill_requirement ? 0x30 + g_cr_blue
                                                       : 0x30 + g_cr_gold);
    secretcolor =
        (fullsecretcount >= totalsecret ? 0x30 + g_cr_blue : 0x30 + g_cr_gold);
    itemcolor =
        (fullitemcount >= totalitems ? 0x30 + g_cr_blue : 0x30 + g_cr_gold);
    kill_percent_color = (kill_percent_count >= totalkills ? 0x30 + g_cr_blue
                                                           : 0x30 + g_cr_gold);
    kill_percent =
        (totalkills == 0 ? 100 : kill_percent_count * 100 / totalkills);
    if (playerscount < 2) {
      snprintf(dsda_exhud_max_totals.msg, sizeof(dsda_exhud_max_totals.msg),
...
void dsda_AddSplit(dsda_split_class_t split_class) {
  ...
  snprintf(dsda_split.msg, DSDA_SPLIT_SIZE, "%d:%05.2f - %s", minutes, seconds,
           split_state->msg);

...
static DWORD L_Associate(const char *Name, const char *Ext,
                         const char *cmdline) {
  HKEY hKeyRoot, hKey;
  DWORD result;
...
static void L_CommandOnChange(void) {
  ...
  case 1: {
    size_t i;
    for (i = 0; i < sizeof(launcher_history) / sizeof(launcher_history[0]);
         i++) {
      char str[32];
      default_t *history;
  ...
  case 5: {
    ...
    launcher_enable_t launcher_next_mode =
        (launcher_enable + 1) % launcher_enable_count;
  ...
  if ((f = fopen(filename, "rb"))) {
    ...
    if (item->source != source_err) {
      ...
      if (0 == fseek(f, LittleLong(header.infotableofs), SEEK_SET)) {
        for (i = 0; !item->doom1 && !item->doom2 && i < (size_t)header.numlumps;
             i++) {
          filelump_t lump;
        ...
        for (j = 0; !processed && j < listPWADCount; j++) {
          ...
          if (index != LB_ERR) {
            if (!strcasecmp(launcher.files[index].name, fullpath))
              if (SendMessage(launcher.listPWAD, LB_SETSEL, true, j) !=
                  CB_ERR) {
                if (topindex == -1)
                  topindex = j;
                L_SelAdd(index);
...
static dboolean L_PrepareToLaunch(void) {
  ...
  if (history) {
    ...
    size_t historycount =
        sizeof(launcher_history) / sizeof(launcher_history[0]);
          ...
          if (!(I_FindAttr(&findstate) & FA_DIREC)) {
            ...
            if (L_GetFileType(fullpath, &item)) {
              if (item.source != source_err) {
                ...
                if (!present) {
                  launcher.files =
                      realloc(launcher.files, sizeof(*launcher.files) *
...
static void L_FillHistoryList(void) {
  ...
  for (i = 0; i < sizeof(launcher_history) / sizeof(launcher_history[0]); i++) {
    if (strlen(launcher_history[i]) > 0) {
      ...
      if (p) {
        index =
            (int)SendMessage(launcher.listHistory, CB_ADDSTRING, 0, (LPARAM)p);
        if (index >= 0)
...
BOOL CALLBACK LauncherClientCallback(HWND hDlg, UINT message, WPARAM wParam,
  ...
  case WM_INITDIALOG: {
    ...
    launcher.staticFileName =
        GetDlgItem(launcher.HWNDClient, IDC_FULLFILENAMESTATIC);

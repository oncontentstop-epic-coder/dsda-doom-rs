...
static void R_MapPlane(int y, int x1, int x2, draw_span_vars_t *dsvars) {
  ...
  // [RH]Instead of using the xtoviewangle array, I calculated the fractional
  // values at the middle of the screen, then used the calculated ds_xstep and
  // ds_ystep to step from those to the proper texture coordinate to start
  // drawing at. That way, the texture coordinate is always calculated by its
  // position on the screen and not by its position relative to the edge of the
  // visplane.
  //
  ...
  dsvars->xstep =
      (fixed_t)((int_64_t)viewsin * planeheight * viewfocratio / den);
  dsvars->ystep =
      (fixed_t)((int_64_t)viewcos * planeheight * viewfocratio / den);

  // killough 2/28/98: Add offsets
  dsvars->xfrac = viewx + xoffs + FixedMul(viewcos, distance) +
                  (x1 - centerx) * dsvars->xstep;
  dsvars->yfrac = -viewy + yoffs - FixedMul(viewsin, distance) +
                  (x1 - centerx) * dsvars->ystep;
    ...
    if (pl->picnum == skyflatnum || pl->picnum & PL_SKYFLAT) { // sky flat
      ...
      if (pl->picnum & PL_SKYFLAT) {
        ...
        if (skystretch) {
          ...
          dcvars.texturemid = (int)((int_64_t)dcvars.texturemid * skyheight /
                                    SKYSTRETCH_HEIGHT);
        }
      ...
      if (heretic) {
        ...
        case 4: // Scroll_EastLavaDamage
          dsvars.source =
              dsvars.source + (((63 - ((leveltime >> 1) & 63)) << 3) & 63);
          break;
        }
      }

...
dboolean P_SetMobjState(mobj_t *mobj, statenum_t state) {
  ...
  if (heretic)
    return Heretic_P_SetMobjState(mobj, state);
...
static void P_XYMovement(mobj_t *mo) {
  ...
  if (mo->z > mo->floorz && !(mo->flags & MF_FLY) && !(mo->flags2 & MF2_FLY) &&
  ...
  if (mo->momx > -STOPSPEED && mo->momx < STOPSPEED && mo->momy > -STOPSPEED &&
    ...
    if (player) {
      if (player->chickenTics) {
        if ((unsigned)(player->mo->state - states - HERETIC_S_CHICPLAY_RUN1) <
            4) {
...
floater:
  ...
  if (mo->z <= mo->floorz) {
      ...
      if (mo->flags & MF_MISSILE) {
        ...
        if (mo->flags2 & MF2_FLOORBOUNCE) {
          ...
          return;
                   ...
                   HERETIC_MT_MNTRFX2) { // Minotaur floor fire can go up steps
          return;
        } else {
          P_ExplodeMissile(mo);
          return;
    ...
    if (mo->flags & MF_SKULLFLY &&
        (!comp[comp_soul] || (compatibility_level > doom2_19_compatibility &&
    ...
    if (mo->momz < 0) {
      ...
      if (mo->flags & MF_TOUCHY && mo->intflags & MIF_ARMED && mo->health > 0)
        ...
        if (mo->player && /* killough 5/12/98: exclude voodoo dolls */
            ...
            // complevels, because original sources do not exclude voodoo dolls
            // from condition above, but Boom does it.
...
void P_MobjThinker(mobj_t *mobj) {
  ...
  else if (!heretic && !(mobj->momx | mobj->momy) &&
           !sentient(mobj)) {    // non-sentient objects at rest
...
mobj_t *P_SpawnMapThing(const mapthing_t *mthing, int index) {
  ...
  if (thingtype <= 4 && thingtype > 0) // killough 2/26/98 -- fix crashes
  {
    ...
    if (!netgame && thingtype > 1 && thingtype <= dogs + 1 &&
        !players[thingtype - 1]
             .secretcount) { // use secretcount to avoid multiple dogs in case
                             // of multiple starts
...
spawnit:
  ...
  if (heretic)
    return mobj; // check below irrelevant
  ...
  if (!((~mobj->flags) &
        (MF_SOLID |
         MF_SPAWNCEILING)) // solid and hanging
...
void P_SpawnPuff(fixed_t x, fixed_t y, fixed_t z) {
  ...
  if (heretic)
    return Heretic_P_SpawnPuff(x, y, z);
...
mobj_t *P_SpawnPlayerMissile(mobj_t *source, mobjtype_t type) {
  ...
  if (comperr(comperr_freeaim))
    ...
    do {
      ...
      if (!linetarget)
        slope =
            P_AimLineAttack(source, an += 1 << 26, 16 * 64 * FRACUNIT, mask);
      if (!linetarget)
        slope =
            P_AimLineAttack(source, an -= 2 << 26, 16 * 64 * FRACUNIT, mask);
      if (!linetarget) {
        an = source->angle;
        slope = 0;

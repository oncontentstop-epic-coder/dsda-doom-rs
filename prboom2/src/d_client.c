...
void NetUpdate(void) {
  ...
  if (server) { // Receive network packets
    ...
    while ((recvlen = I_GetPacket(packet, 10000))) {
      ...
      case PKT_BACKOFF:
        /* cph 2003-09-18 -
         * The server sends this when we have got ahead of the other clients. We
         * should stall the input side on this client, to allow other clients to
         * catch up.
         */
        lastmadetic++;
        break;

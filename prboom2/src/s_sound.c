...
void S_UpdateSounds(void *listener_p) {
  ...
  if (heretic)
    return Heretic_S_UpdateSounds(listener_p);
...
static dboolean S_StopSoundInfo(sfxinfo_t *sfx, int priority) {
  ...
  for (i = 0; i < numChannels; i++) {
    if (channels[i].sfxinfo == sfx && channels[i].origin) {
      ...
      if (priority >=
          channels[i].priority) { // if we're gonna kill one, then this'll be it
        least_priority = i;
...
static void Heretic_S_StartSound(void *_origin, int sound_id) {
  ...
  for (i = 0; i < numChannels; i++) {
    ...
    if (origin == channels[i].origin) { // only allow other mobjs one sound
  ...
  channels[i].handle =
      I_StartSound(sound_id, i, vol, sep, channels[i].pitch, priority);
...
static void Heretic_S_StartSoundAtVolume(void *_origin, int sound_id,
  ...
  channels[i].handle =
      I_StartSound(sound_id, i, volume, 128, channels[i].pitch, 1);
  channels[i].origin = origin;
  channels[i].sfxinfo = sfx;

...
#define CHECK_WEAPON_CODEPOINTER(codepointer, player)                          \
  if (!player->mo->player) {                                                   \
    I_Error("%s: Weapon codepointers cannot be used with player/monster "      \
...
static void P_FireWeapon(player_t *player) {
  ...
  if (heretic) {
    ...
    wpinfo = player->powers[pw_weaponlevel2] ? &wpnlev2info[0] : &weaponinfo[0];
...
void A_BeakReady(player_t *player, pspdef_t *psp) {
  if (player->cmd.buttons & BT_ATTACK) { // Chicken beak attack
    ...
    if (player->mo->state ==
        &states[HERETIC_S_CHICPLAY_ATK1]) { // Take out of attack state
...
void A_FireGoldWandPL2(player_t *player, pspdef_t *psp) {
  ...
  P_SpawnMissileAngle(mo, HERETIC_MT_GOLDWANDFX2, mo->angle - (ANG45 / 8),
                      momz);
  P_SpawnMissileAngle(mo, HERETIC_MT_GOLDWANDFX2, mo->angle + (ANG45 / 8),
                      momz);
...
void A_FireMacePL1(player_t *player, pspdef_t *psp) {
  ...
  ball =
      P_SPMAngle(player->mo, HERETIC_MT_MACEFX1,
...
void A_SkullRodStorm(mobj_t *actor) {
  ...
  if (P_Random(pr_heretic) < 25) { // Fudge rain frequency
    return;
  }
...
void A_RemovedPhoenixFunc(mobj_t *actor) { return; }
...
void P_CloseWeapons(void) {
  ...
  if (!deathmatch && P_Random(pr_heretic) <
                         64) { // Sometimes doesn't show up if not in deathmatch
    return;
  }
  spot = P_Random(pr_heretic) % MaceSpotCount;

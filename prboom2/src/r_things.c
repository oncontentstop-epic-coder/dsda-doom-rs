...
#define R_SpriteNameHash(s)                                                    \
  ((unsigned)((s)[0] - ((s)[1] * 3 - (s)[3] * 2 - (s)[2]) * 2))
...
static void R_ProjectSprite(mobj_t *thing, int lightlevel) {
  ...
  if (thing->flags & g_mf_shadow)
    ...
    int index =
        (int)(((int_64_t)xscale * 160 / wide_centerx) >> LIGHTSCALESHIFT);
...
static void R_DrawPSprite(pspdef_t *psp) {
  ...
  if (weapon_attack_alignment && viewplayer->attackdown && !psp->state->misc1) {
    ...
    R_ApplyWeaponBob(&psp_sx, weapon_attack_alignment == CENTERWEAPON_BOB, NULL,
                     false);
  ...
  if (movement_smooth) {
    ...
    if (lump == psp_inter.lump) {
      ...
      vis->x1 =
          psp_inter.x1 + FixedMul(tic_vars.frac, (vis->x1 - psp_inter.x1));
...
#define bcopyp(d, s, n)                                                        \
  asm(" cld; rep; movsl;" ::"D"(d), "S"(s), "c"(n)                             \
      : "%cc", "%esi", "%edi", "%ecx", "memory")
...
static void msort(vissprite_t **s, vissprite_t **t, int n) {
  if (n >= 16) {
    ...
    while ((*s1)->scale > (*s2)->scale ? (*d++ = *s1++, --n1)
                                       : (*d++ = *s2++, --n2))
      ;
...
static void R_DrawSprite(vissprite_t *spr) {
  ...
  if (spr->heightsec != -1) // only things in specially marked sectors
  {
    ...
    if ((mh = sectors[spr->heightsec].floorheight) > spr->gz &&
      ...
      if (mh <= 0 ||
          (phs != -1 && viewz > sectors[phs].floorheight)) { // clip bottom
        for (x = spr->x1; x <= spr->x2; x++)

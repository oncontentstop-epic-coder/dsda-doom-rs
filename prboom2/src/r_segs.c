...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2) {
  ...
  if (curline->linedef->tranlump >= 0 && general_translucency) {
    colfunc = R_GetDrawColumnFunc(RDC_PIPELINE_TRANSLUCENT, drawvars.filterwall,
                                  drawvars.filterz);
  ...
  for (dcvars.x = x1; dcvars.x <= x2; dcvars.x++, spryscale += rw_scalestep)
    if (maskedtexturecol[dcvars.x] != INT_MAX) // dropoff overflow
    {
      ...
      dcvars.texu =
          ds->rw_offset - FixedMul(finetangent[angle], ds->rw_distance);
  ...
  curline = NULL; /* cph 2001/11/18 - must clear curline now we're done with it,
...
static void R_RenderSegLoop(void) {
  ...
  for (; rw_x < rw_stopx; rw_x++) {
    ...
    if (segtextured) {
      ...
      if (!fixedcolormap) {
        int index =
            (int)(((int_64_t)rw_scale * 160 / wide_centerx) >> LIGHTSCALESHIFT);
        if (index >= MAXLIGHTSCALE)
          index = MAXLIGHTSCALE - 1;
  ...
  } else // two sided line
  {
    ...
    if (worldhigh < worldtop) // top texture
    {
      ...
      rw_toptexturemid = linedef->flags & ML_DONTPEGTOP
                             ? worldtop
                             : backsector->ceilingheight +
                                   textureheight[sidedef->toptexture] - viewz;
      rw_toptexturemid +=
          FixedMod(sidedef->rowoffset, textureheight[toptexture]);
    }
    ...
    if (worldlow > worldbottom) // bottom texture
    {
      ...
      rw_bottomtexturemid +=
          FixedMod(sidedef->rowoffset, textureheight[bottomtexture]);
    }
